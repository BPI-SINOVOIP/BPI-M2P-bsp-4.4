...
#ifdef CONFIG_SUN8IW7P1
#include <../arch/arm/include/asm/arch-sun8iw7p1/efuse.h>
#endif
...
void update_bootargs(void)
{
	...
	//securemode info
	sprintf(tmpbuf," is_secure=%d",(int)gd->securemode);
	strcat(cmdline,tmpbuf);

#ifdef CONFIG_SUN8IW7P1
        u32 value;
        value = sid_read_key(EFUSE_CHIPID) & 0x0ff;
        printf("the chip id is 0x%x\n", value);
        sprintf(tmpbuf," chipid=%x",value);
        strcat(cmdline,tmpbuf);
#endif
