...
int __sunxi_burn_key(u8 *buff, uint buff_len)
{
	...
	for(;key_count>0;key_count--, key_list++)
	{
		...
		else
		{
			...
			{
				/*!!For security reason, not allowed to burn clear widevine key now.
				* it will ignore the config item from burn tool global config file
				*/
				key_list->if_crypt = 1;
...
int __sunxi_read_key_by_name(void *buffer, uint buff_len, int *read_len)
{
	...
	*read_len = sizeof(sunxi_usb_burn_key_info_t);
	...
	else
	{
		...
		ret = read_private_key_by_name(key_info->name, data_buff,
			sizeof(sunxi_usb_burn_key_info_t), &key_data_len);
		if (ret < 0)
		{
			printf("read %s form private  failed\n", key_info->name);
			return -1;/*now we can makesure both sec storage && private is wrong*/
		}
		else {
			key_info->len = key_data_len;
			memcpy((void *)(key_info->key_data),
