...
#define  NAND_DRV_DATE			0x20170817
...
//rerurn 1:secure mode
extern int sunxi_is_file_mode_toc(void);

int NAND_IS_Secure_sys(void)
{
	int mode=0;
	#if defined(CONFIG_ARCH_SUN8IW7P1)
	mode = !!(sunxi_get_securemode() | sunxi_is_file_mode_toc());
	#else
	mode = sunxi_get_securemode();
	#endif

	tick_printf("NAND_IS_Secure_sys: mode %d\n", mode);
	if(mode==0)
		return 0;
	else
		return 1;
