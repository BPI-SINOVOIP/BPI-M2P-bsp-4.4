	...
	for (i = 0; i < toc1_head->items_nr; i++, toc1_item++) {
		...
		if (strncmp(toc1_item->name, ITEM_UBOOT_NAME,
			...
			toc1_flash_read(toc1_item->data_offset / 512,
                        ...
                        *(uint *)(SUNXI_SYSCONFIG_SIZE_ADDR) = toc1_item->data_len;
			toc1_flash_read(toc1_item->data_offset / 512,
                                        (toc1_item->data_len + 511) / 512,
                ...
                } else if (strncmp(toc1_item->name, ITEM_CPUS_CODE_NAME,
                            sizeof(ITEM_CPUS_CODE_NAME)) == 0) {
                    *(uint *)(SUNXI_CPUS_CODE_SIZE_ADDR) = toc1_item->data_len;
                    toc1_flash_read(toc1_item->data_offset / 512,
                            (toc1_item->data_len + 511) / 512,
