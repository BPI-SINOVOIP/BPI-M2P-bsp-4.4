...
/* struct dma_des *pnfc_desc = (struct dma_des*)DMAC_DES_BASE; */
	...
	pnfc_desc->config = DMA_CFG_SRC_DRQ_NAND0 | DMA_CFG_SRC_BST1_WIDTH32 |
			    DMA_CFG_SRC_SECURITY | DMA_CFG_SRC_IO |
			    DMA_CFG_DST_DRQ_SDRAM | DMA_CFG_DST_BST1_WIDTH32 |
			    DMA_CFG_DST_SECURITY | DMA_CFG_DST_LINEAR;
			...
			__u8 *in_inverse, __u32 len)
{
	__u32 bit, byte;
	__u32
	    cnt_1; /* the total number of bit '1' on specified bit position in
		      all input bytes */
	__u32 cnt_0;
...
__s32 NFC_ReadRetry(__u32 chip, __u32 retry_count, __u32 read_retry_type)
{
	...
	if (read_retry_mode < 0x10) {
		if ((read_retry_mode == 0) || (read_retry_mode == 1)) {
			...
			/*		    for debug
				    {
					  __u32 j;
					  __u8
			   param_tmp[READ_RETRY_MAX_REG_NUM];
					PHY_DBG("readretry check, retry count =
			   %d\n", retry_count);
					PHY_DBG("table value: \n");
					for(i=0;i<read_retry_reg_num;i++)
					    PHY_DBG(" 0x%x", param[i]);
					PHY_DBG("\n");
					ret =_vender_get_param(&param_tmp[0],
			   &read_retry_reg_adr[0], read_retry_reg_num);
			...
			/*		    for debug
			   ...
			   param_tmp[READ_RETRY_MAX_REG_NUM];
					...
					ret =_vender_get_param(&param_tmp[0],
			   &read_retry_reg_adr[0], read_retry_reg_num);
					PHY_DBG("get value after set value cycle
			   %d: \n", j);
					for(i=0;i<read_retry_reg_num;i++)
					    PHY_DBG(" 0x%x", param_tmp[i]);
					PHY_DBG("\n");
		...
		} else if ((read_retry_mode == 4) || (read_retry_mode == 6)) {
			for (i = 0; i < read_retry_reg_num; i++)
				param[i] =
...
__s32 NFC_ReadRetryInit(__u32 read_retry_type)
{
	...
	} else if (read_retry_mode == 6) {
    	/* mode6  H27UCG8T2FTR */
		read_retry_reg_adr[0] = 0x0E;
		read_retry_reg_adr[1] = 0x0F;
		read_retry_reg_adr[2] = 0x10;
		read_retry_reg_adr[3] = 0x11;
	} else if (read_retry_mode == 0x40) {
...
void NFC_GetOTPValue(__u32 chip, __u8 *otp_value, __u32 read_retry_type)
{
	...
	if ((read_retry_mode == 0x2) || (read_retry_mode == 0x3)) {
		...
		for (i = 0; i < 64; i++)
			pdata[i] = otp_value[i];
	} else if ((read_retry_mode == 0x4) || (read_retry_mode = 0x6)) {
