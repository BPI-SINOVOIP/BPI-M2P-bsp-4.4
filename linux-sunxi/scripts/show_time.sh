...
do
	...
	if [ $? == 0 ]; then
		...
		duration=`echo $endcall | awk -F'after' '{print $2}' | awk '{print $1}'`
		printf "[%-8f] %-46s %-10d\n" $callstamp $module $duration
	else
		printf "Error: can't find endcall of $s\n" $module
		printf "line content is: $endcall"
		sleep 10
		continue
	fi
...
printf "total boot time: %fs, total initcall time(including mod init): %dus\n" $boottime $inittime
