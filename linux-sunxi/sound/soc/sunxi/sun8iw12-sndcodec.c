...
extern void __iomem *codec_analogadress;
static ssize_t acodec_read(struct file *filep, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (!buf)
		return -ENOMEM;
	codec = snd_soc_sunxi_sndpcm.rtd[0].codec;
	ret = codec->driver->read(codec, SUNXI_DAC_DBG_CTRL);
	acodecvol = (ret & 0x3f)*116 - 7308;
	ret = codec->driver->read(codec, SUNXI_DAC_VOL_CTRL);
	dacrvol = (ret & 0xff)*75 - 12000;
	daclvol = ((ret >> 8)&0xff)*75 - 12000;
	ret = codec->driver->read(codec, SUNXI_ADC_VOL_CTRL);
	adcrvol = (ret & 0xff)*75 - 12000;
	adclvol = ((ret >> 8)&0xff)*75 - 12000;
	ret = read_prcm_wvalue(LINEOUT_CTRL1, codec_analogadress);
	outvol = (ret & 0x1f) * 15 - 435;
	if (outvol > 0)
		outvol = 0;
	ret = read_prcm_wvalue(MIC1_CTRL, codec_analogadress);
	mic1vol = (ret & 7)*3+21;
	if (mic1vol < 24)
		mic1vol = 0;
	if (ret & 8)
		...
		mic1boost = 0;
	ret = read_prcm_wvalue(MIC2_CTRL, codec_analogadress);
	mic2vol = (ret & 7)*3+21;
	if (mic2vol < 24)
		mic2vol = 0;
	if (ret & 8)
		...
		mic2boost = 0;
	ret = 0;
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "ACodecVol:%d\n", acodecvol);
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "AdcLVol:%d\n", adclvol);
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "AdcRVol:%d\n", adcrvol);
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "DacLVol:%d\n", daclvol);
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "DacRVol:%d\n", dacrvol);
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "OutVol:%d\n", outvol);
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "Mic1Vol:%d\n", mic1vol);
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "Mic1Boost:%d\n", mic1boost);
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "Mic2Vol:%d\n", mic2vol);
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "Mic2Boost:%d\n", mic2boost);
...
static ssize_t acodecin_read(struct file *filep, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (!buf)
		return -ENOMEM;
	l_src = read_prcm_wvalue(L_ADCMIX_SRC, codec_analogadress);
	if (l_src & 0x64) {
		...
		l_id = 0;
	r_src = read_prcm_wvalue(R_ADCMIX_SRC, codec_analogadress);
	if (r_src & 0x64) {
	...
	ret = 0;
	for (i = 0; i < 4; i++) {
		rt = snd_soc_sunxi_sndpcm.rtd[i].pcm->streams[1].substream->runtime;
		if (!rt)
...
static ssize_t acodecout_read(struct file *filep, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	ret = 0;
	for (i = 0; i < 4; i++) {
		rt = snd_soc_sunxi_sndpcm.rtd[i].pcm->streams[0].substream->runtime;
		if (!rt)
			ret += snprintf(buf+ret, PAGE_SIZE-ret, "stream %d inactive\n", i);
		else {
