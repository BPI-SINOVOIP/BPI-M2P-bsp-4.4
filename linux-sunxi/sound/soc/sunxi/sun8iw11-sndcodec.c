/*
 * sound\soc\sunxi\sunx8iw11-sndcodec.c
...
static int sunxi_card_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_disable_pin(&card->dapm, "Radio");
	snd_soc_dapm_disable_pin(&card->dapm, "Headphone");
	snd_soc_dapm_disable_pin(&card->dapm, "Phoneout Speaker");
	snd_soc_dapm_disable_pin(&card->dapm, "LINEIN");
	snd_soc_dapm_disable_pin(&card->dapm, "LINEOUT");
	...
	if (priv->jack_gpio > 0) {
		...
		ret = snd_soc_card_jack_new(card, "Headphone Jack",
					SND_JACK_HEADPHONE,
					&priv->jack_detect, sunxi_jack_pins,
		...
		if (ret) {
			dev_err(card->dev,
				"jack_add_gpios failed: %d, gpio has't eint\n",
				ret);
			return ret;
		}
	}
