...
{
	struct sunxi_codec *sunxi_internal_codec = snd_soc_codec_get_drvdata(dai->codec);
	...
	case	SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
			regmap_update_bits(sunxi_internal_codec->regmap, SUNXI_DAC_FIFO_CTR,
					(1<<DAC_DRQ_EN), (1<<DAC_DRQ_EN));
		else
			regmap_update_bits(sunxi_internal_codec->regmap, SUNXI_ADC_FIFO_CTR,
