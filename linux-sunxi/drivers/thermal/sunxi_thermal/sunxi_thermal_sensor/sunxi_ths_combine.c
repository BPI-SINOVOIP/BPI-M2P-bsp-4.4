...
sunxi_ths_controller_register(struct device *dev, struct sunxi_ths_controller_ops *ops, void *data)
{
	...
	combine_name = kzalloc(sizeof(*combine_name), GFP_KERNEL);
	if (IS_ERR_OR_NULL(combine_name)) {
		pr_err("combine_name: not enough memory for combine_name data\n");
		return NULL;
	}
