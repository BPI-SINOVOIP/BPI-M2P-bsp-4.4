				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	bw = pfile->f_op->write(pfile,
				(const char *)buf_addr, TVD_DBG_DUMP_LEN,
				&pfile->f_pos);
...
static struct attribute *v4in1_attributes[] = {
...
static int __tvd_clk_init(struct tvd_dev *dev)
{
	...
	dev->parent = clk_get_parent(tvd_clk[0]);
	if (IS_ERR_OR_NULL(dev->parent) || IS_ERR_OR_NULL(tvd_clk[0]))
		return -EINVAL;
	...
	pr_debug("%s: parent = %lu, clk = %lu\n", __func__,
		 clk_get_rate(dev->parent), clk_get_rate(tvd_clk[0]));
...
static int __tvd_config(struct tvd_dev *dev)
{
	...
	tvd_set_wb_width_and_jump(dev->sel, dev->width / 2);

	if (dev->interface == YPBPRP_INTERFACE)
		tvd_set_wb_height(dev->sel, dev->height);	/*P,no div */
	else
		tvd_set_wb_height(dev->sel, dev->height / 4);
...
static void __tvd_set_addr(struct tvd_dev *dev, struct tvd_buffer *buffer)
{
	...
	tvd_set_wb_addr(dev->sel, addr_org, dev->width, dev->height);
}
...
static irqreturn_t tvd_isr(int irq, void *priv)
{
	...
	u32 err = (1 << TVD_IRQ_FIFO_C_O) |
	    (1 << TVD_IRQ_FIFO_Y_O) |
	    (1 << TVD_IRQ_FIFO_C_U) |
	    (1 << TVD_IRQ_FIFO_Y_U) | (1 << TVD_IRQ_WB_ADDR_CHANGE_ERR);

	if (dev->special_active == 1)
		return v4in1_isr_special(irq, priv);
	...
	if (!waitqueue_active(&buf->vb.vb2_queue->done_wq))
		pr_debug("%s: None is waiting for buf(0x%p)\n", __func__, buf);
...
static const struct vb2_ops v4in1_video_qops = {
				...
				struct v4l2_format *f)
{
	...
	if (dev->height == 576 * 2) {
		 ...
		 * According to the hardware designer, tvd need 29.7M
...
_3d_fliter:
	...
	if (dev->fliter.used) {
		...
		if (fliter_count < FLITER_NUM) {
			if (__tvd_3d_comp_mem_request(dev, (int)
						TVD_3D_COMP_BUFFER_SIZE)) {
...
static int tvd_open(struct file *file)
{
	...
	/* register irq */
	ret = request_irq(dev->irq, tvd_isr, 0, "v4in1", dev);
...
static int tvd_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	struct tvd_dev *dev = container_of(ctrl->handler, struct tvd_dev,
					   ctrl_handler);
...
int v4in1_info_special(void)
{
	return tvd_count;
}
EXPORT_SYMBOL(v4in1_info_special);

int v4in1_open_special(int tvd_fd)
{
...
EXPORT_SYMBOL(v4in1_open_special);

int v4in1_close_special(int tvd_fd)
{
	struct tvd_dev *dev = tvd;
	int ret = 0;
	int i = 0;
...
EXPORT_SYMBOL(v4in1_close_special);

int v4in1_s_fmt_vid_cap_special(int tvd_fd, struct v4l2_format *f)
{
	struct tvd_dev *dev = tvd;
	int ret = 0;
...
EXPORT_SYMBOL(v4in1_s_fmt_vid_cap_special);

int v4in1_g_fmt_vid_cap_special(int tvd_fd, struct v4l2_format *f)
{
	struct tvd_dev *dev = tvd;
	u32 locked = 0, system = 2;
...
EXPORT_SYMBOL(v4in1_g_fmt_vid_cap_special);

int v4in1_dqbuffer_special(int tvd_fd, struct tvd_buffer **buf)
{
	int ret = 0;
	unsigned long flags = 0;
	struct tvd_dev *dev = tvd;
...
EXPORT_SYMBOL(v4in1_dqbuffer_special);

int v4in1_qbuffer_special(int tvd_fd, struct tvd_buffer *buf)
{
	struct tvd_dev *dev = tvd;
...
EXPORT_SYMBOL(v4in1_qbuffer_special);

int v4in1_streamon_special(int tvd_fd, enum v4l2_buf_type i)
{
	struct tvd_dev *dev = tvd;
...
EXPORT_SYMBOL(v4in1_streamon_special);

int v4in1_streamoff_special(int tvd_fd, enum v4l2_buf_type i)
{
	struct tvd_dev *dev = tvd;
	struct tvd_dmaqueue *dma_q = &dev->vidq_special;
...
EXPORT_SYMBOL(v4in1_streamoff_special);

static void (*tvd_buffer_done)(int tvd_fd);
void v4in1_register_buffer_done_callback(void *func)
{
	pr_debug("%s\n", __func__);
	tvd_buffer_done = func;
}
EXPORT_SYMBOL(v4in1_register_buffer_done_callback);
...
static const struct v4l2_file_operations v4in1_fops = {
	.owner = THIS_MODULE,
	.open = tvd_open,
	.release = tvd_close,
	.read = tvd_read,
...
static const struct v4l2_ioctl_ops v4in1_ioctl_ops = {
...
static int __tvd_init_controls(struct v4l2_ctrl_handler *hdl)
{
	...
	v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops, V4L2_CID_BRIGHTNESS,
			  0, 255, 1, 128);
...
static int __v4in1_probe_init(struct platform_device *pdev)
{
	...
	for (i = 0; i < 4; i++) {
		/* fetch tvd0~tvd3 and map addr. */
		tvd_addr[i] = of_iomap(pdev->dev.of_node, i + 1);
	...
	sprintf(dev->v4l2_dev.name, "v4in1_v4l2_dev%d", 0);
	...
	*vfd = tvd_template[0];
	...
	dev->vfd = vfd;
	pr_info("v4in1 device registered as %s\n", video_device_node_name(vfd));
	...
	ret = sysfs_create_group(&dev->pdev->dev.kobj, &tvd_attribute_group[0]);
	if (ret) {
...
static int v4in1_probe(struct platform_device *pdev)
{
	int ret = 0, i = 0;
	struct device_node *np = pdev->dev.of_node;
	char sub_name[32] = { 0 };
...
{				/*fix */
	...
	pr_debug("v4in1_release ok!\n");

	return 0;
}

static int v4in1_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_group(&dev->pdev->dev.kobj, &tvd_attribute_group[0]);
...
#ifdef CONFIG_PM_RUNTIME
static int v4in1_runtime_suspend(struct device *d)
{
	return 0;
}

static int v4in1_runtime_resume(struct device *d)
{
	return 0;
}

static int v4in1_runtime_idle(struct device *d)
{
	if (d) {
...
static int v4in1_suspend(struct device *d)
{
	return 0;
}

static int v4in1_resume(struct device *d)
{
	return 0;
}

static void v4in1_shutdown(struct platform_device *pdev)
{

}

static const struct dev_pm_ops v4in1_runtime_pm_ops = {
#ifdef CONFIG_PM_RUNTIME
	.runtime_suspend = v4in1_runtime_suspend,
	.runtime_resume = v4in1_runtime_resume,
	.runtime_idle = v4in1_runtime_idle,
#endif
	.suspend = v4in1_suspend,
	.resume = v4in1_resume,
};

static const struct of_device_id sunxi_v4in1_match[] = {
	{.compatible = "allwinner,sunxi-v4in1",},
	{},
};

static struct platform_driver v4in1_driver = {
	.probe = v4in1_probe,
	.remove = v4in1_remove,
	.shutdown = v4in1_shutdown,
	.driver = {
		   .name = "sunxi-v4in1",
		   .owner = THIS_MODULE,
		   .of_match_table = sunxi_v4in1_match,
		   .pm = &v4in1_runtime_pm_ops,
		   }
};

static int __init v4in1_module_init(void)
{
	int ret;
	...
	ret = platform_driver_register(&v4in1_driver);
	if (ret) {
...
static void __exit v4in1_module_exit(void)
{
	pr_info("tvd_exit\n");

	v4in1_release();
	platform_driver_unregister(&v4in1_driver);
}

module_init(v4in1_module_init);
module_exit(v4in1_module_exit);
