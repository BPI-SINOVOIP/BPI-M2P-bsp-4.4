...
static void sample_save_frame(struct work_struct *work)
{
	...
	if (IS_ERR_OR_NULL(fp)) {
		printk("open %s failed!, ERR NO is %ld.\n", file_path_isp, (long)fp);
		goto qbuf;
	}
	for (i = 0; i < BUF_NUM; ++i) {
	...
	if (ret < 0)
		printk("file_write failed!, ERR NO is %d.\n", ret);

	ret = filp_close(fp, NULL);
	if (ret < 0)
		printk("file close failed!, ERR NO is %d.\n", ret);
qbuf:
...
static int sample_req_buffers(void)
{
	...
	for (i = 0; i < BUF_NUM; ++i) {
		...
		if (os_mem_alloc(&ion_buf[i]) < 0) {
			printk("alloc ion buffer failed\n");
			return -1;
		}
		video_buf[i].paddr = ion_buf[i].dma_addr;

		if (vfe_qbuffer_special(vfe_id, &video_buf[i])) {
			printk("vfe_qbuffer_special failed\n");
			return -1;
		}
	}
	return 0;
...
static int sample_video_open(int sel)
{
	if (vfe_open_special(vfe_id) < 0) {
		printk("vfe_open_special falied\n");
		return -1;
	}

	if (vfe_s_input_special(vfe_id, 0) < 0) {
		printk("vfe_s_input_special %d error!\n", sel);
		return -1;
	}
...
static int sample_fmt_set(void)
{
	...
	if (vfe_s_fmt_special(vfe_id, &fmt) < 0) {
		printk("vfe_s_fmt_special error!\n");
		return -1;
	}

	if (vfe_g_fmt_special(vfe_id, &fmt) < 0) {
		printk("vfe_g_fmt_special error!\n");
		return -1;
	} else {
		printk("resolution got from sensor = %d*%d\n",
...
int sample_start(void)
{
	...
	if (vfe_streamon_special(vfe_id, V4L2_BUF_TYPE_VIDEO_CAPTURE) < 0) {
		printk("vfe_streamon_special failed\n");
		return -1;
	} else {
		printk("vfe_streamon_special ok\n");
	}
...
int sample_end(void)
{
	if (vfe_streamoff_special(vfe_id, V4L2_BUF_TYPE_VIDEO_CAPTURE) < 0) {
		printk("vfe_streamoff_special failed\n");
		return -1;
	} else {
		printk("vfe_streamoff_special ok\n");
	}
	...
	if (vfe_close_special(vfe_id) < 0) {
		printk("vfe_close_special falied\n");
		return -1;
	}
