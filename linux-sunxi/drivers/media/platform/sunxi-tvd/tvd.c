...
static ssize_t tvd_read(struct file *file, char __user *data,
...
static int tvd_open(struct file *file)
{
	...
	dev->system = NTSC;
	/* register irq */
	ret = request_irq(dev->irq, tvd_isr, 0, dev->name, dev);
...
{
	int ret = 0;
	struct tvd_dev *dev = container_of(ctrl->handler,
				struct tvd_dev, ctrl_handler);
...
static int __tvd_init_controls(struct v4l2_ctrl_handler *hdl)
{
	...
	v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops,
				V4L2_CID_BRIGHTNESS, 0, 255, 1, 128);
	v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops,
				V4L2_CID_CONTRAST, 0, 128, 1, 0);
	v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops,
...
static char extcon_lock_name[20];
static char extcon_system_name[20];
static struct extcon_dev extcon_lock[TVD_MAX];
static struct extcon_dev extcon_system[TVD_MAX];
...
static int __tvd_auto_plug_init(struct tvd_dev *dev)
{
	...
	snprintf(extcon_lock_name, sizeof(extcon_lock_name), "tvd_lock%d",
		dev->sel);

	extcon_lock[dev->sel].name = extcon_lock_name;

	extcon_lock[dev->sel].supported_cable = tvd_cable;

	ret = extcon_dev_register_attr(&extcon_lock[dev->sel], &dev->pdev->dev);


	snprintf(extcon_system_name, sizeof(extcon_system_name), "tvd_system%d",
		dev->sel);

	extcon_system[dev->sel].name = extcon_system_name;
	extcon_system[dev->sel].supported_cable = tvd_cable;
	ret = extcon_dev_register_attr(&extcon_system[dev->sel],
							&dev->pdev->dev);
...
{
	extcon_dev_unregister(&extcon_lock[dev->sel]);
	extcon_dev_unregister(&extcon_system[dev->sel]);
}
	...
	for (;;) {
		...
		for (i = 0; i < tvd_count; i++) {
			...
			if (hpd[i] != locked) {
				...
				hpd[i] = locked;
				extcon_set_state(&extcon_lock[i], locked);
			}
			if (systems[i] != system) {
				pr_debug("system = %d, i = %d\n", system, i);
				systems[i] = system;
				extcon_set_state(&extcon_system[i], system);
			}
		}
...
static int tvd_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < tvd_num; i++) {
		...
		if (!sub_pdev) {
			dev_err(&pdev->dev,
					"fail to find device for tvd%d!\n", i);
			continue;
		}
		if (sub_pdev) {
			ret = __tvd_probe_init(i, sub_pdev);
			if (ret != 0) {
				/* one tvd may init fail because
				 * of the sysconfig
				 */
