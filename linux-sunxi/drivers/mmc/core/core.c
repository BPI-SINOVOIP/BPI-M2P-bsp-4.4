...
void mmc_manual_remove_card(struct mmc_host *host, struct mmc_card *card)
{
		...
		/*
		*disable any detect work,
		*for that card->state may be change by rescan
		*/
		spin_lock_irqsave(&host->lock, flags);
		host->rescan_disable = 1;
		spin_unlock_irqrestore(&host->lock, flags);
		...
		/*
		*We maual set card remove for that some wifi will response
		*to our cmd even it has been reseted
		*We claim host here for that card->state may be change
		*by other theoretically
		*so we claim here for save
		*/
