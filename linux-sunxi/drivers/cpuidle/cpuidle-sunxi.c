...
{
	if (cpu == 0)
		WARN_ON(cpu0_entry_flag != value);
	else if (cpu == 1)
		WARN_ON(cpu1_entry_flag != value);
	else if (cpu == 2)
		WARN_ON(cpu2_entry_flag != value);
	else if (cpu == 3)
		WARN_ON(cpu3_entry_flag != value);
}
...
static int sunxi_idle_entry(unsigned long val)
{
#if defined(CONFIG_SUNXI_ARISC)
	struct sunxi_idle_para idle_para;
	int cpu;
	...
	idle_para.cpu = cpu;
	idle_para.level = val;
	idle_para.resume_addr = (void *)virt_to_phys(cpu_resume);

	arisc_enter_cpuidle(NULL, NULL, &idle_para);
#else
	...
	sunxi_idle_core_power_down(val);

	return 0;
}

static int sunxi_idle_enter(struct cpuidle_device *dev,
				struct cpuidle_driver *drv, int idx)
{
	int ret;
#if defined CONFIG_ARCH_SUN8IW12P1
	unsigned int target_cpu = smp_processor_id();;
	...
	ret = cpu_suspend(idx, sunxi_idle_entry);
...
static int sunxi_cpuidle_probe(struct platform_device *pdev)
{
	int ret;
...
module_platform_driver(sunxi_cpuidle_driver);
