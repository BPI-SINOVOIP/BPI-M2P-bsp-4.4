...
int sunxi_tr_commit(unsigned int id, tr_info *info)
{
	...
	pr_info("in: fmt=%d, pitch=%d,%d,%d, rect=%d,%d,%d,%d\n",
	    info->src_frame.fmt, info->src_frame.pitch[0],
	    info->src_frame.pitch[1], info->src_frame.pitch[2],
	    info->src_rect.x, info->src_rect.y, info->src_rect.w,
	    info->src_rect.h);
	pr_info("out: fmt=%d, pitch=%d,%d,%d, rect=%d,%d,%d,%d\n",
	    info->dst_frame.fmt, info->dst_frame.pitch[0],
	    info->dst_frame.pitch[1], info->dst_frame.pitch[2],
	    info->dst_rect.x, info->dst_rect.y, info->dst_rect.w,
	    info->dst_rect.h);
#endif
