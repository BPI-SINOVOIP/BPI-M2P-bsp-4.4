...
int de_dcsc_apply(unsigned int sel, struct disp_csc_config *config)
{
	...
	config->enhance_mode = (config->enhance_mode > CSC_ENHANCE_MODE_NUM - 1) ? g_dcsc_config[sel].enhance_mode :  config->enhance_mode;
	...
	de_csc_coeff_calc(config->in_fmt, config->in_mode,\
						config->out_fmt, config->out_mode,\
