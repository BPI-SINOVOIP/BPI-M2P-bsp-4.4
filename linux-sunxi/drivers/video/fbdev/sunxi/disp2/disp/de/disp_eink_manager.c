...
static s32 eink_calculate_index_data(struct disp_eink_manager *manager)
{
	...
	__debug
	    ("new inde=%d,old_index=%d, old_index_data_paddr=%p, new_index_data_paddr=%p\n",
...
int disp_init_eink(struct disp_bsp_init_para *para)
{
	...
	eink_manager =
	    kmalloc(sizeof(struct disp_eink_manager) * MAX_EINK_ENGINE,
	...
	eink_private_data =
	    kmalloc(sizeof(struct eink_private) * MAX_EINK_ENGINE,
		    GFP_KERNEL | __GFP_ZERO);
