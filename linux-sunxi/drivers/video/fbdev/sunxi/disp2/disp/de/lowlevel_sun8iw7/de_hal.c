...
static int de_set_coarse(unsigned int sel, unsigned char chno, unsigned int fmt, unsigned int lcd_fps,
						 ...
						 struct scaler_para *fix_ypara, struct scaler_para *fix_cpara, unsigned char yv12_4k_enable)
{
	int coarse_status;
...
static int de_calc_overlay_scaler_para(unsigned int screen_id, unsigned char chn, unsigned char layno, unsigned char *fmt,
									   ...
									   unsigned char *premode, struct de_rect (*crop)[LAYER_MAX_NUM_PER_CHN], struct de_rect (*layer)[LAYER_MAX_NUM_PER_CHN],
									   struct de_rect *bld_rect, unsigned int *ovlw, unsigned int *ovlh, unsigned char *pen, struct scaler_para *ovl_para,
									   struct scaler_para *ovl_cpara)
{
	...
	static struct scaler_para para[CHN_NUM][LAYER_MAX_NUM_PER_CHN], cpara[VI_CHN_NUM][LAYER_MAX_NUM_PER_CHN],
	...
	/*init para*/
	for (j = 0; j < vi_chn; j++)
		memset((void *)cpara[j], 0x0, layno*sizeof(struct scaler_para));
	for (j = 0; j < chn; j++)
		memset((void *)para[j], 0x0, layno*sizeof(struct scaler_para));
	...
	for (j = 0, k = 0; j < chn; j++) {
		for (i = 0; i < layno; ) {
			...
			if (data[k].config.info.fb.flags) {
				...
				de_rtmx_get_3d_in_single_size((enum de_3d_in_mode)data[k].config.info.fb.flags, &crop64[j][i]);
				if (data[k].config.info.b_trd_out) {
					de_rtmx_get_3d_in_single_size((enum de_3d_in_mode)data[k].config.info.fb.flags, &crop64[j][i + 1]);
					de_rtmx_get_3d_out(frame[j][i], lcd_width, lcd_height, (enum de_3d_out_mode)data[k].config.info.out_trd_mode, &frame[j][i + 1]);
	...
	for (j = 0; j < vi_chn; j++) {
		...
		if (scaler_en)
			de_vsu_sel_ovl_scaler_para(lay_en[j], para[j], cpara[j], &ovl_para[j], &ovl_cpara[j]);   /*recalculate overlay size, blending coordinate, blending size, layer coordinate*/
	...
	for (j = 0, k = 0; j < chn; j++) {
		for (i = 0; i < layno; ) {
			...
			if (data[k].config.info.fb.flags) {
				...
				de_rtmx_get_3d_in(data[k].config.info.fb.format, crop[j][i+1], (struct de_fb *)data[k].config.info.fb.size, data[k].config.info.fb.align,
									(enum de_3d_in_mode)data[k].config.info.fb.flags, lay_cfg[k].laddr_t, data[k].config.info.fb.trd_right_addr,
	...
	for (j = 0, k = 0; j < chn; j++) {
		...
		for (i = 0; i < layno; i++) {
			if (LAYER_ATTR_DIRTY & data[k].flag) {
				...
				data[k].config.info.fb.align, (enum de_3d_in_mode)data[k].config.info.fb.flags, lay_cfg[k].laddr_t, haddr[i]);
			}
