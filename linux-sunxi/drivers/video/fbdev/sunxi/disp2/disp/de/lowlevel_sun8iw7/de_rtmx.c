...
static de_reg_blocks vi_attr_block[DEVICE_NUM][VI_CHN_NUM][LAYER_MAX_NUM_PER_CHN];
static de_reg_blocks vi_fc_block[DEVICE_NUM][VI_CHN_NUM];
static de_reg_blocks vi_haddr_block[DEVICE_NUM][VI_CHN_NUM];
static de_reg_blocks vi_size_block[DEVICE_NUM][VI_CHN_NUM];

static de_reg_blocks ui_attr_block[DEVICE_NUM][CHN_NUM-VI_CHN_NUM][LAYER_MAX_NUM_PER_CHN];
static de_reg_blocks ui_haddr_block[DEVICE_NUM][CHN_NUM-VI_CHN_NUM];
static de_reg_blocks ui_size_block[DEVICE_NUM][CHN_NUM-VI_CHN_NUM];

static de_reg_blocks bld_attr_block[DEVICE_NUM];
static de_reg_blocks bld_ctl_block[DEVICE_NUM];
static de_reg_blocks bld_ck_block[DEVICE_NUM];
static de_reg_blocks bld_out_block[DEVICE_NUM];
static void __iomem *de_base;

extern int de_calc_ovl_coord(unsigned int frame_coord, unsigned int scale_step, int vsu_sel);
int de_rtmx_trimcoord(struct de_rect *frame, struct de_rect *crop, unsigned int outw, unsigned int outh, int xratio, int yratio);
 ...
 *              unsigned int chno, unsigned int layno, struct struct __lay_para_t *cfg)
	...
	struct __lay_para_t *cfg)
{
	...
	if (chno >= vi_chno) {
		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].
			lay_attr.bits.lay_en = cfg->en;
		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].
			lay_attr.bits.lay_alpmod = cfg->alpha_mode;
		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].
			lay_attr.bits.lay_fcolor_en = cfg->fcolor_en;
		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].
			lay_attr.bits.lay_fmt = cfg->fmt;
		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].
			lay_attr.bits.lay_alpctl = cfg->premul_ctl;
		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].
			lay_attr.bits.lay_top_down = cfg->top_bot_en;
		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].
			lay_attr.bits.lay_alpha     = cfg->alpha;

		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].
			lay_size.bits.lay_width =
			(cfg->layer.w == 0) ? 0 : (cfg->layer.w - 1);
		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].
			lay_size.bits.lay_height =
			(cfg->layer.h == 0) ? 0 : (cfg->layer.h - 1);

		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].
			lay_coor.bits.lay_coorx = cfg->layer.x;
		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].
			lay_coor.bits.lay_coory = cfg->layer.y;
		...
		de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[layno].lay_size.bits.lay_width =
				cfg->layer.w == 0 ? 0 : cfg->layer.w - 1;
		de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[layno].lay_size.bits.lay_height =
				cfg->layer.h == 0 ? 0 : cfg->layer.h - 1;
	...
	unsigned char *haddr_t, unsigned char *haddr_b)
{
	...
	if (chno >= vi_chno) {
		...
		if (top_bot_en) {
			...
			de200_rtmx[sel].ui_ovl[chno-vi_chno]->ui_lay_bot_haddr.dwval =
 ...
 *                 unsigned int layno, unsigned char fmt,struct struct de_rect crop,
 *                 unsigned int *size, enum enum de_3d_in_mode trdinmode,
	...
	unsigned int *addr, unsigned char *haddr)
{
	...
	if (chno >= vi_chno) {
		...
		if (trdinmode == DE_3D_SRC_MODE_LI) {
			addr_off[1] =
				addr[1] + de_rtmx_get_li_addr_offset(
				size[1], align[1], x1, y1, ucnt);
			addr_off[2] =
				addr[2] + de_rtmx_get_li_addr_offset(
				size[2], align[2], x1, y1, ucnt);
		} else {
...
int de_rtmx_get_3d_out(struct de_rect frame0,
	unsigned int scn_w, unsigned int scn_h,
	enum de_3d_out_mode trdoutmode, struct de_rect *frame1)
{
	switch (trdoutmode) {
...
static int de_rtmx_get_coarse_fac(unsigned int ovl_w, unsigned int ovl_h,
	unsigned int vsu_outw, unsigned int vsu_outh, unsigned int fmt,
	...
	unsigned int *midch, unsigned char yv12_4k_en)
{
	...
	unsigned char yv12_4k_en)
{
	...
	status = de_rtmx_get_coarse_fac(ovl_w, ovl_h, vsu_outw, vsu_outh,
		fmt, lcd_fps, lcd_height, de_freq_MHz,
		&yhm, &yhn, &yvm, &yvn, &chm, &chn, &cvm, &cvn,
		midyw, midyh, midcw, midch, yv12_4k_en);
...
int de_rtmx_set_route(unsigned int sel, unsigned char pno, unsigned int zoder)
{
	...
	route = de200_rtmx[sel].bld_ctl->bld_route_ctl.dwval
		& (~(0xf << (pno << 2)));
	...
	unsigned int pre_mul)
{
	...
	pre_mode = de200_rtmx[sel].bld_ctl->bld_premultiply_ctl.dwval
		& (~(0x1 << pno));
...
int de_rtmx_calc_chnrect(unsigned char *lay_en, int laynum,
	struct de_rect *frame, struct de_rect *crop, int gsu_sel, struct scaler_para *step,
	struct de_rect *layer, struct de_rect *bld_rect, unsigned int *ovlw, unsigned int *ovlh)
{
...
int de_rtmx_sync_hw(unsigned int sel)
{
	...
	memcpy((void *)glb_ctl_block[sel].val,
		(void *)glb_ctl_block[sel].off,
		glb_ctl_block[sel].size);
	...
	memcpy((void *)bld_attr_block[sel].val,
		(void *)bld_attr_block[sel].off,
		bld_attr_block[sel].size);

	memcpy((void *)bld_ctl_block[sel].val,
		(void *)bld_ctl_block[sel].off,
		bld_ctl_block[sel].size);

	memcpy((void *)bld_ck_block[sel].val,
		(void *)bld_ck_block[sel].off,
		bld_ck_block[sel].size);

	memcpy((void *)bld_out_block[sel].val,
