...
struct composer_private_data {
	...
	u32 last_display_cnt[CNOUTDISPSYNC];
	u32 primary_disp;
	bool b_no_output;
	bool wb_status;
	struct mutex sync_lock;
	int disp_hotplug_cnt[CNOUTDISPSYNC];
	/* 0 is de0 ,1 is de1 , 2 is writeback,3 is FB */
	struct display_sync *display_sync[CNOUTDISPSYNC];
	struct disp_layer_config *tmp_hw_lyr;
	struct disp_drv_info *psg_disp_drv;
...
static s32 composer_get_frame_fps(u32 type)
{
	...
	cur_time_index =
	    (pre_time_index == 0) ? (DBG_TIME_SIZE - 1) : (pre_time_index - 1);
...
unsigned int composer_dump(char *buf)
{
	...
	return sprintf(buf,
		       "acquire: %d, %d.%d fps\nrelease: %d, %d.%d fps\ndisplay: %d, %d.%d fps\n",
		       cnt0, fps0 / 10, fps0 % 10, cnt1, fps1 / 10, fps1 % 10,
		       cnt2, fps2 / 10, fps2 % 10);
}
...
static void imp_finish_cb(bool force_all)
{
	...
	for (i = 0; i < CNOUTDISPSYNC; i++) {
		while (composer_priv.display_sync[i] != NULL
		       ...
		       && composer_priv.display_sync[i]->timeline->value !=
		       composer_priv.cur_write_cnt[i]) {
			if (!force_all &&
			    composer_priv.display_sync[i]->timeline->value + 1
			    == composer_priv.cur_disp_cnt[i]) {
				break;
			}
			sw_sync_timeline_inc(composer_priv.display_sync[i]->
					     timeline, 1);
...
int dispc_gralloc_queue(struct disp_layer_config *commit_layer,
			unsigned int disp, unsigned int hwc_sync,
			struct disp_capture_info *data)
{
	struct disp_drv_info *psg_disp_drv = composer_priv.psg_disp_drv;
	...
	if (disp_mgr != NULL) {
		...
		disp_mgr->set_layer_config(disp_mgr, commit_layer,
					   disp ? 8 : 16);
	...
	if (disp_device && (disp_device->is_enabled(disp_device) == 0))
...
unsigned int hwc_get_sync(int sync_disp, int fd)
{
	...
	if (fence == NULL) {
		pr_warn("hwc get relesefence(%d) err.\n", fd);
		goto err;
	}
	sync_fence_put(fence);
#endif
	if (!list_empty(&fence->pt_list_head)) {
		pt = list_entry(fence->pt_list_head.next, struct sync_pt,
				pt_list);
	} else {
		pr_warn("hwc get sw_pt err\n");
		goto err;
	}
...
static bool hwc_fence_get(void *user_fence)
{
	...
	if (copy_from_user
	    (&fd, (void __user *)user_fence, sizeof(int) * CNOUTDISPSYNC)) {
		pr_warn("copy_from_user hwc_fence_get err.\n");
		goto fecne_ret;
	}
	for (i = 0; i < CNOUTDISPSYNC; i++) {
		if (fd[i] == HWC_SYNC_NEED) {
			if (composer_priv.display_sync[i] == NULL) {
				ret =
				    sprintf(buf, "sunxi_%d_%d", i,
					    composer_priv.disp_hotplug_cnt[i]);
				composer_priv.display_sync[i] =
				    kzalloc(sizeof(struct display_sync),
					    GFP_KERNEL);
				if (composer_priv.display_sync[i] == NULL) {
					pr_warn(
					       "kzalloc the dispay_sync err.\n");
					continue;
				}
				composer_priv.display_sync[i]->timeline =
				    sw_sync_timeline_create(buf);
				if (composer_priv.display_sync[i]->timeline
				    == NULL) {
					kfree(composer_priv.display_sync[i]);
					composer_priv.display_sync[i] = NULL;
					pr_warn("creat timeline err.\n");
					continue;
				}
			...
			if (pt == NULL) {
				put_unused_fd(fd[i]);
				pr_warn("creat display pt fail\n");
				continue;
			}
			fence = sync_fence_create(buf, pt);
			if (fence == NULL) {
				put_unused_fd(fd[i]);
				pr_warn("creat dispay fence fail\n");
				continue;
			}
...
static int hwc_commit(void *user_display, bool compat)
{
	...
	if (compat) {
		ret =
		    copy_from_user(&disp_compat_data,
				   (void __user *)user_display,
				   sizeof(struct hwc_compat_dispc_data));
		if (ret) {
			pr_warn(
			       "hwc copy_from_user hwc_compat_dispc_data err.\n");
			goto commit_ok;
		}
	...
	{
		ret =
		    copy_from_user(&disp_data, (void __user *)user_display,
				   sizeof(struct hwc_dispc_data));
		if (ret) {
			pr_warn(
			       "hwc copy_from_user hwc_dispc_data err.\n");
			goto commit_ok;
		}
		for (i = 0; i < DISP_NUMS_SCREEN; i++) {
			hwc_layer_info[i] =
			    (unsigned long)disp_data.hwc_layer_info[i];
	...
	if (releasefencefd[composer_priv.primary_disp] >= 0
	    && !hwc_pridisp_sync()) {
	...
	if (NULL != (void *)wb_ptr_data && !force_flip[0]) {
		if (copy_from_user(&wb_data, (void __user *)wb_ptr_data,
		     sizeof(struct disp_capture_info))) {
			pr_warn(
			       "hwc copy_from_user write back data err.\n");
		}
		need_wb = 1;
	}
	for (i = 0; i < DISP_NUMS_SCREEN; i++) {
		if (releasefencefd[i] >= 0 && !force_flip[i]) {
			if (copy_from_user(composer_priv.tmp_hw_lyr,
			     (void __user *)hwc_layer_info[i],
			     sizeof(struct disp_layer_config) * (i ? 8 : 16))) {
				pr_warn(
				       "hwc copy_from_user layer_config err\n");
				ret = -1;
				continue;
			}
			dispc_gralloc_queue(composer_priv.tmp_hw_lyr, i,
					    sync[i], (need_wb
						      && (i == 0)) ?
						      &wb_data : NULL);
		}
...
static int hwc_compat_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == DISP_HWC_COMMIT) {
		...
		if (copy_from_user
		    (&hwc_ctl, (void __user *)ubuffer[1],
		     sizeof(struct hwc_compat_ioctl_arg))) {
...
static void disp_composer_proc(u32 sel)
{
	...
	if (sel < 2) {
		if (sel == 0 && composer_priv.wb_status == 1) {
			...
			wb_status =
			    &composer_priv.check_wb[composer_priv.cur_disp_cnt[sel] %
						    WB_CHECK_SIZE];
			wb_status->sync = composer_priv.cur_disp_cnt[sel];
			wb_status->success = !wb_back->query(wb_back);
		}
		composer_priv.last_display_cnt[sel] =
		    composer_priv.cur_disp_cnt[sel];
		composer_priv.cur_disp_cnt[sel] =
		    composer_priv.cur_write_cnt[sel];
	}
	schedule_work(&composer_priv.post2_cb_work);
}
...
s32 composer_init(struct disp_drv_info *psg_disp_drv)
{
	...
	composer_priv.tmp_hw_lyr =
	    kzalloc(sizeof(struct disp_layer_config) * 16, GFP_KERNEL);

	if (composer_priv.tmp_hw_lyr == NULL)
		pr_warn(
		       "hwc_composer init err when kzalloc,you need reboot\n");
