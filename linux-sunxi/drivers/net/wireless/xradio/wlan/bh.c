...
int xradio_bh_resume(struct xradio_common *hw_priv)
{
	...
	ret = wait_event_timeout(hw_priv->bh_evt_wq, (hw_priv->bh_error ||
	     ...
	     0 : -ETIMEDOUT;
	if (ret) {
		bh_printk(XRADIO_DBG_WARN, "%s, bh resume failed, try again\n", __func__);
#ifdef BH_USE_SEMAPHORE
		up(&hw_priv->bh_sem);
#else
		wake_up(&hw_priv->bh_wq);
...
static u32 xradio_realtime_interval_us(struct timeval *oldtime, struct timeval *newtime)
{
	u32 time_int;
...
#ifdef XRADIO_SAVE_SOFTWAVE
		 interval_us = xradio_realtime_interval_us(&start_time, &end_time);
		 if (hw_priv->debug)
			 debug_save_item(hw_priv, &DbgSaveItems, 10, 2, rx_total_cnt, next_rx_cnt, tx_total_cnt, rx,
							   tx, interval_us, hw_priv->bh_tx,
							   hw_priv->tx_queue[2].num_queued, hw_priv->tx_queue[2].num_pending);
#endif
...
rx:
			...
			do_gettimeofday(&start_time);
			if (hw_priv->debug)
				hw_priv->debug->glb_tx_cfm_cnt = 0;
#endif
			...
			/* trigger here, save */
			if (hw_priv->debug)
				debug_save_item(hw_priv, &DbgSaveItems, 10, 0, rx_total_cnt, next_rx_cnt, tx_total_cnt, rx_burst,
							  hw_priv->debug->glb_tx_cfm_cnt, interval_us, hw_priv->bh_tx,
							  hw_priv->tx_queue[2].num_queued, hw_priv->tx_queue[2].num_pending);
#endif
...
tx:
		...
		if (tx && tx_allowed) {
			...
			if (ret <= 0) {
				...
				interval_us = xradio_realtime_interval_us(&start_time, &end_time);
				if (hw_priv->debug)
					debug_save_item(hw_priv, &DbgSaveItems, 10, 1, rx_total_cnt, next_rx_cnt, tx_total_cnt,
							  (hw_priv->wsm_caps.numInpChBufs - hw_priv->hw_bufs_used - 1),
							  0, interval_us, hw_priv->bh_tx,
							  hw_priv->tx_queue[2].num_queued, hw_priv->tx_queue[2].num_pending);
#endif
