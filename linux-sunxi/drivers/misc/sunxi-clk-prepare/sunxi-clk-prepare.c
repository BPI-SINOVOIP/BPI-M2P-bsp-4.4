...
int sunxi_clk_enable_prepare(struct clk *clk)
{
	...
	for (i = 0; i < CLK_MAX_MODULE_VALUE; i++) {
		if (clk_bitmap_name_mapping[i].mask_bit != 0) {
			if (!strcmp(clk_bitmap_name_mapping[i].id_name,
						clk->name)) {
	...
	if (mask_bit == 0) {
		pr_err("%s: donot find related module\n", __func__);
		return 0;
	}
	...
	for (i = 0; i < CLK_MAX_ID_VALUE; i++) {
		if ((1<<i) & mask_bit) {
			...
			if (IS_ERR_OR_NULL(clk_handle)) {
				pr_err("%s: try to get %s failed!\n",
						__func__, id_name[i]);
				continue;
			}
			if (clk_prepare_enable(clk_handle)) {
				pr_err("%s: try to enable %s failed!\n",
						__func__, id_name[i]);
				clk_put(clk_handle);
				continue;
			}
...
int sunxi_clk_disable_prepare(struct clk *clk)
{
	...
	for (i = 0; i < CLK_MAX_MODULE_VALUE; i++) {
		if (clk_bitmap_name_mapping[i].mask_bit != 0) {
			if (!strcmp(clk->name,
				clk_bitmap_name_mapping[i].id_name)) {
				mask_bit = clk_bitmap_name_mapping[i].mask_bit;
				break;
			}
		}
...
static unsigned int sunxi_clk_check_flags(unsigned int mask_bit)
{
	...
	for (i = 0; i < CLK_MAX_ID_VALUE; i++) {
		if ((1<<i) & mask_bit) {
			...
			if (!np) {
				dprintk(DEBUG_INIT,
					"%s: failed to find node %s\n",
					__func__, dts_module_id[i]);
				continue;
			}
			if (strcmp("codec", dts_module_id[i]) == 0) {
				if (of_property_read_u32(np,
						"headphone_en", &out_value))
					continue;
				else {
					if (out_value == 0)
						mask_bit &= ~(1<<i);
...
static int __init sunxi_clk_prepare_init(void)
{
	...
	for (i = 0; i < CLK_MAX_MODULE_VALUE; i++) {
		if (clk_bitmap_name_mapping[i].mask_bit != 0)
			clk_bitmap_name_mapping[i].mask_bit =
				sunxi_clk_check_flags(
					clk_bitmap_name_mapping[i].mask_bit);
	}
