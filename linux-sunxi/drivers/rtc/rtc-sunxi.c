...
int sunxi_rtc_set_bootup_extend_mode(int mode)
{
	...
	np = of_find_compatible_node(NULL, NULL, "allwinner,sun8i-rtc");
#else
	...
	if (mode == SUNXI_BOOTUP_EXTEND_MODE_POWEROFF)
		writel(SUNXI_BOOTUP_EXTEND_RTC_POWEROFF, chip->base + SUNXI_GP_DATA_REG2);
	else if (mode == SUNXI_BOOTUP_EXTEND_MODE_RESTART)
		writel(SUNXI_BOOTUP_EXTEND_RTC_RESTART, chip->base + SUNXI_GP_DATA_REG2);
	else
