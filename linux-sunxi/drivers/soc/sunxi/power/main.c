...
module_param_named(time_to_wakeup, time_to_wakeup,
	ulong, S_IRUGO | S_IWUSR);

static unsigned int wakeup_events;
module_param_named(wakeup_events, wakeup_events,
	uint, S_IRUGO);

extern char sunxi_bromjump_start;
extern char sunxi_bromjump_end;

static const struct extended_standby_manager_t *est_manager_id;

extern unsigned long cpu_brom_addr[2];
#if (defined CONFIG_TEE)
extern phys_addr_t sunxi_smc_get_teeaddr_paras(phys_addr_t resumeaddr);
#endif
static inline int sunxi_mem_get_status(void)
{
	return sunxi_readl(SUNXI_RTC_PBASE + 0x104);
}

static inline void sunxi_mem_set_status(int val)
{
	sunxi_writel(val, SUNXI_RTC_PBASE + 0x104);
	asm volatile ("dsb");
	asm volatile ("isb");
}
...
static int sunxi_pm_valid(suspend_state_t state)
{
	...
	else if (state == PM_SUSPEND_MEM || state == PM_SUSPEND_BOOTFAST)
...
static int sunxi_pm_begin(suspend_state_t state)
{
	...
	static int backup_debug_mask;
	static int fail_num_back;
	static int need_write_back;

	if (fail_num_back < suspend_stats.fail) {
		pr_warn("last suspend err!, rtc:%x\n",
...
{
	int i;
	struct super_standby_para st_para;

	sunxi_mem_set_status(SUSPEND_ENTER | 0x03);

	standby_level = STANDBY_WITH_POWER_OFF;

	memset((void *)&st_para, 0, sizeof(struct super_standby_para));
	...
	st_para.resume_entry = virt_to_phys(&sunxi_bromjump_start);
#if (defined CONFIG_TEE)
	st_sram_para.resume_code_src =
		sunxi_smc_get_teeaddr_paras(virt_to_phys(cpu_resume));
#else
	...
	if (unlikely(debug_mask)) {
		...
		pr_info("system environment paras:\n");
...
static unsigned long sunxi_back_gic_secure(struct regs_restore *regback)
{
	...
	for (i = 0; i < 0x08; i += 4, regback++) {
		...
		regback->value = sunxi_readl(SUNXI_GIC_CPU_VBASE + 0x0 + i);
		num++;
	}

	/* back GICD_CTLR */
	regback->addr = SUNXI_GIC_DIST_PBASE;
	regback->value = sunxi_readl(SUNXI_GIC_DIST_VBASE);
	regback++;
	...
	for (i = 0; i < 0x18; i += 4, regback++) {
		...
		regback->value =
			sunxi_readl(SUNXI_GIC_DIST_VBASE + 0x80 + i);
		num++;
	}
...
{
	pr_warn("suspend failure!\n");

	sunxi_mem_set_status(SUSPEND_FAIL_FLAG);
}


/* define platform_suspend_ops call-back functions,
* registered into PM core */
static const struct platform_suspend_ops sunxi_pm_ops = {
...
static int sunxi_pm_syscore_suspend(void)
{
	...
	cpuclk_config = sunxi_readl(SUNXI_CCM_PBASE + 0x50);
	ahbclk_config = sunxi_readl(SUNXI_CCM_PBASE + 0x54);

	pll_cpu = clk_get(NULL, PLL_CPU_CLK);
	if (IS_ERR(pll_cpu)) {
		pr_err("back pll_cpu failed!\n");
		return -EINVAL;
	}
	...
	if (unlikely(debug_mask)) {
		pr_info("cpu pll&clk config:%lu %x ahb clk config:%x\n",
...
static void sunxi_pm_syscore_resume(void)
{
	...
	value = sunxi_readl(SUNXI_CCM_PBASE + 0x50);
	/* set CPU_APB_CLK_DIV, bit8~9 */
	value &= ~(0x03 << 8);
	value |= (cpuclk_config & (0x03 << 8));
	sunxi_writel(value, SUNXI_CCM_PBASE + 0x50);
	udelay(10);
	...
	sunxi_writel(cpuclk_config, SUNXI_CCM_PBASE + 0x50);
	mdelay(1);
	sunxi_mem_set_status(CLK_RESUME_START | 3);

	/* restore AHB1/APB1 Configuration Register
	* from low to high frequency */
	value = sunxi_readl(SUNXI_CCM_PBASE + 0x54);
	/* set AHB1_PRE_DIV, bit6~7 */
	value &= ~(0x03 << 6);
	value |= (ahbclk_config & (0x03 << 6));
	sunxi_writel(value, SUNXI_CCM_PBASE + 0x54);
	udelay(10);
	...
	sunxi_writel(ahbclk_config, SUNXI_CCM_PBASE + 0x54);
	mdelay(2);
	...
	pll_cpu = clk_get(NULL, PLL_CPU_CLK);
	if (IS_ERR(pll_cpu)) {
		pr_err("get pll_cpu failed!\n");
		return;
	}
	if (clk_set_rate(pll_cpu, pll_cpu_back)) {
		pr_err("set pll_cpu failed!\n");
		return;
	}
	if (clk_prepare_enable(pll_cpu)) {
		pr_err("enable pll_cpu failed!\n");
		return;
	}
	clk_put(pll_cpu);
}
