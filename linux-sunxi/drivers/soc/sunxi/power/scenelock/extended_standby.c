...
static int copy_extended_standby_data(
	struct scene_extended_standby_t *standby_data)
{
	int i = 0;
	...
	if (!standby_data) {
		...
		if ((0 != temp_standby_data.id) &&
			...
			if (0 != standby_data->extended_standby_data.pll_change) {
				for (i = 0; i < PLL_NUM; i++) {
					if (standby_data->extended_standby_data.pll_change &
						(0x1<<i)) {
						if (!(temp_standby_data.pll_change & (0x1<<i)))
							temp_standby_data.pll_factor[i] =
						standby_data->extended_standby_data.pll_factor[i];
			...
			if (0 != standby_data->extended_standby_data.bus_change) {
				for (i = 0; i < BUS_NUM; i++) {
					if (standby_data->extended_standby_data.bus_change &
						(0x1<<i)) {
						if (!(temp_standby_data.bus_change & (0x1<<i)))
							temp_standby_data.bus_factor[i] =
						standby_data->extended_standby_data.bus_factor[i];
 ...
 */
bool set_extended_standby_manager(
	struct scene_extended_standby_t *local_standby)
{
	...
	if (local_standby && 0 ==
		local_standby->extended_standby_data.pwr_dm_en) {
		return true;
	}
 ...
 *	to get the corresponding wakeup src intterupt state, enable or disable.
 *
 *	@src:			wakeup src.
 *	@para:			if wakeup src need para, be the para of wakeup src,
 *				else ignored.
 ...
 *	show current standby state, for debug purpose.
 *
...
int extended_standby_show_state(void)
{
	...
	spin_lock_irqsave(&data_lock, irqflags);
	pr_info("wakeup_src 0x%lx\n",
		extended_standby_manager.event);
	pr_info("wakeup_gpio_map 0x%lx\n",
		extended_standby_manager.wakeup_gpio_map);
	pr_info("wakeup_gpio_group 0x%lx\n",
		extended_standby_manager.wakeup_gpio_group);
	if (NULL != extended_standby_manager.pextended_standby) {
		pr_info("extended_standby id = 0x%lx\n",
			extended_standby_manager.pextended_standby->id);
		if (extended_standby_manager.pextended_standby->pll_change != 0) {
			for (i = 0; i < PLL_NUM; i++) {
...
unsigned int parse_wakeup_event(char *s, unsigned int size,
	unsigned int event)
{
	int i = 0;
	int count = 0;
	...
	unsigned int gpio_map)
{
	...
	uk_printf(s, end - s, "%s", "WAKEUP_GPIO,for cpus:pl,pm, \
		and axp, is as follow: \n");
	...
	unsigned int group_map)
{
	...
	uk_printf(s, end - s, "WAKEUP_GPIO,for cpux:pa,pb,pc,pd,.., \
		is as follow: \n");
	for (i = 0; i < 32; i++) {
...
module_param_named(parse_bitmap_en, parse_bitmap_en, uint,
	S_IRUGO | S_IWUSR);
