...
int arisc_sysconfig_ir_paras(void)
{
	...
	for (i = 0; i < 8; i++) {
		...
		if (of_property_read_u32(np, subkey, &ir_power_key_code)) {
			ARISC_ERR("parse ir power key code fail\n");
			return -EINVAL;
		}

		sprintf(subkey, "%s%d", "ir_addr_code", i);
		if (of_property_read_u32(np, subkey, &ir_addr_code)) {
			ARISC_WRN("parse ir address code fail\n");
			return -EINVAL;
		}
		result = arisc_config_ir_paras(ir_power_key_code, ir_addr_code);
		if (result)
			return result;
	}
