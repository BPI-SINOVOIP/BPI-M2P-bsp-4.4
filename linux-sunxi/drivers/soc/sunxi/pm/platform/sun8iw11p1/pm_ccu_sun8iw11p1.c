/*
 * drivers/soc/sunxi/pm/platform/sun8iw11p1/pm_ccu_sun8iw11p1.c
 * (C) Copyright 2010-2016
 * Allwinner Technology Co., Ltd. <www.allwinnertech.com>
 * Yanggq <yanggq@allwinnertech.com>
 *
 * ccu operation for sun8iw11p1
 *
...
{
	ccm_reg->ccm_reg = (struct __ccmu_reg_list_t *) IO_ADDRESS(AW_CCM_BASE);
#if 0
	/* bias: */
	...
	memcpy(&(ccm_reg->ccm_reg_backup),
				ccm_reg->ccm_reg,
				sizeof(struct __ccmu_reg_list_t));
#endif
...
__s32 mem_ccu_restore(struct ccm_state *ccm_reg)
{
	...
	/*bias: 0x218*/
	ccm_reg->ccm_reg->PllSataBias = ccm_reg->ccm_reg_backup.PllSataBias;
	/*0x21c, periph1 hsic bias reg */
	/*ccm_reg->ccm_reg->PllPeriph1Bias = ccm_reg->ccm_reg_backup.PllPeriph1Bias;*/
	/*0x220,  pll cpux  bias reg */
	/*ccm_reg->ccm_reg->PllxBias[0] = ccm_reg->ccm_reg_backup.PllxBias[0];*/
	/*0x224,  pll audio bias reg */
	ccm_reg->ccm_reg->PllAudioBias = ccm_reg->ccm_reg_backup.PllAudioBias;
	/*0x228,  pll vedio bias reg */
	ccm_reg->ccm_reg->PllVideo0Bias = ccm_reg->ccm_reg_backup.PllVideo0Bias;
	/*0x22c,  pll ve    bias reg */
	ccm_reg->ccm_reg->PllVeBias = ccm_reg->ccm_reg_backup.PllVeBias;
	/*0x230, pll dram0 bias reg */
	/*ccm_reg->ccm_reg->PllDram0Bias = ccm_reg->ccm_reg_backup.PllDram0Bias;*/
	/*0x234, pll periph0 bias reg */
	/*ccm_reg->ccm_reg->PllPeriph0Bias = ccm_reg->ccm_reg_backup.PllPeriph0Bias;*/
	/*0x238, pll video1 bias reg */
	...
	/*0x24c,  pll dram1 bias */
	/*ccm_reg->ccm_reg->PllDram1BiasReg = ccm_reg->ccm_reg_backup.PllDram1BiasReg;*/

	/*tune & pattern: */
	/*0x250, pll1 tun,cpux tuning reg */
	ccm_reg->ccm_reg->Pll1Tun = ccm_reg->ccm_reg_backup.Pll1Tun;
	/*0x260, pll ddr0 tuning */
	/*ccm_reg->ccm_reg->PllDdr0Tun = ccm_reg->ccm_reg_backup.PllDdr0Tun;*/
	/*0x70, mipi tuning reg*/
	ccm_reg->ccm_reg->pllMipiTun = ccm_reg->ccm_reg_backup.pllMipiTun;
	/*0x27c, pll pre1ph1 pattern control reg */
	/*ccm_reg->ccm_reg->PllPeriph1Pattern = ccm_reg->ccm_reg_backup.PllPeriph1Pattern;*/
	/*0x280,  pll cpux  pattern reg */
	/*ccm_reg->ccm_reg->Pll1Pattern = ccm_reg->ccm_reg_backup.Pll1Pattern;*/
	/*0x284,  pll audio pattern reg */
	ccm_reg->ccm_reg->PllAudioPattern =
				ccm_reg->ccm_reg_backup.PllAudioPattern;
	/*0x288, pll vedio pattern reg */
	ccm_reg->ccm_reg->PllVedio0Pattern =
				ccm_reg->ccm_reg_backup.PllVedio0Pattern;
	/*0x28c, pll ve pattern reg */
	ccm_reg->ccm_reg->PllVePattern = ccm_reg->ccm_reg_backup.PllVePattern;
	/*0x290, pll ddr0 pattern reg */
	/*ccm_reg->ccm_reg->PllDdr0Pattern = ccm_reg->ccm_reg_backup.PllDdr0Pattern;*/
	/*0x298, pll video1 pattern reg */
	ccm_reg->ccm_reg->PllVedio1Pattern = ccm_reg->ccm_reg_backup.PllVedio1Pattern;
	/*0x29c, pll gpu pattern reg */
	ccm_reg->ccm_reg->PllGpuPattern = ccm_reg->ccm_reg_backup.PllGpuPattern;
	/*0x2a8, pll de pattern reg */
	ccm_reg->ccm_reg->PllDePattern = ccm_reg->ccm_reg_backup.PllDePattern;
	...
	/*cfg all moudules pll must close before enter standby*/
	/*0x008, audio*/
	...
	/*0x020, ddr0*/
	/*ccm_reg->ccm_reg->Pll5Ctl = ccm_reg->ccm_reg_backup.Pll5Ctl;*/
	/*0x028, periph0*/
	ccm_reg->ccm_reg->Pll6Ctl = ccm_reg->ccm_reg_backup.Pll6Ctl;
	/*0x02c, periph1*/
	ccm_reg->ccm_reg->Pll7Ctl = ccm_reg->ccm_reg_backup.Pll7Ctl;
	/*0x0030, Pll videio1 reg */
	ccm_reg->ccm_reg->PllVideo1 = ccm_reg->ccm_reg_backup.PllVideo1;
	/*0x0034, sata ctrl reg */
	ccm_reg->ccm_reg->PllSata = ccm_reg->ccm_reg_backup.PllSata;
	/*0x0038, PLL8 control, gpu */
	ccm_reg->ccm_reg->Pll8Ctl = ccm_reg->ccm_reg_backup.Pll8Ctl;
	/*0x0040, MIPI PLL control */
	ccm_reg->ccm_reg->PllMipi = ccm_reg->ccm_reg_backup.PllMipi;
	/*0x048, de*/
	ccm_reg->ccm_reg->Pll10Ctl = ccm_reg->ccm_reg_backup.Pll10Ctl;
	/*0x004c, pll ddr1 ctrl reg */
	/*ccm_reg->ccm_reg->PllDdr1Ctl = ccm_reg->ccm_reg_backup.PllDdr1Ctl;*/
	change_runtime_env();
	delay_us(10);
	...
	/*notice: be care to cpu, axi restore sequence!. */
	/*0x0050, cpux/axi clock divide ratio */
...
#if 0
	/* do not touch dram related config. */
	ccm_reg->ccm_reg->PllDdrAuxiliary =
				ccm_reg->ccm_reg_backup.PllDdrAuxiliary;
	...
	ccm_reg->ccm_reg->DeinterlaceClk =
				ccm_reg->ccm_reg_backup.DeinterlaceClk;
	...
	ccm_reg->ccm_reg->Sys32kClk = ccm_reg->ccm_reg_backup.Sys32kClk
				| (0x16aa << 16);
