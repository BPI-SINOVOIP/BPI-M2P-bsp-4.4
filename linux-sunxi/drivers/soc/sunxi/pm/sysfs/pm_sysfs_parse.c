/*
 * drivers/soc/sunxi/pm/sysfs/pm_sysfs_parse.c
...
ssize_t parse_pwr_dm_map(char *s, size_t size, unsigned int bitmap)
{
	...
	for (i = 0; i < (pwr_dm_bitmap_name_mapping_cnt); i++) {
		bit_event = ((1 << pwr_dm_bitmap_name_mapping[i].mask_bit)
				& bitmap);
		if (bit_event) {
			uk_printf(s, end - s, "\t\t%s bit 0x%x\t",
				pwr_dm_bitmap_name_mapping[i].id_name,
				(1 << pwr_dm_bitmap_name_mapping[i].mask_bit));
			count++;
		}
...
static unsigned int parse_bitmap_gpio(char *s, unsigned int size,
				unsigned int bitmap)
{
	char *start = s;
	char *end = NULL;
...
unsigned int parse_wakeup_event(char *s, unsigned int size, unsigned int event)
{
	...
	for (i = 0; i < 32; i++) {
		...
		case CPUS_WAKEUP_SHORT_KEY:
			uk_printf(s, end - s, "%-36s bit 0x%x \t ",
				"CPUS_WAKEUP_SHORT_KEY", CPUS_WAKEUP_SHORT_KEY);
			count++;
			break;
		case CPUS_WAKEUP_LONG_KEY:
			uk_printf(s, end - s, "%-36s bit 0x%x \t ",
				"CPUS_WAKEUP_LONG_KEY ", CPUS_WAKEUP_LONG_KEY);
			count++;
			break;
		case CPUS_WAKEUP_IR:
			uk_printf(s, end - s, "%-36s bit 0x%x \t ",
		...
		case CPUS_WAKEUP_USBMOUSE:
			uk_printf(s, end - s, "%-36s bit 0x%x \t ",
				"CPUS_WAKEUP_USBMOUSE ", CPUS_WAKEUP_USBMOUSE);
			count++;
			break;
		...
		case CPUS_WAKEUP_BAT_TEMP:
			uk_printf(s, end - s, "%-36s bit 0x%x \t ",
				"CPUS_WAKEUP_BAT_TEMP ", CPUS_WAKEUP_BAT_TEMP);
			count++;
			break;
		case CPUS_WAKEUP_FULLBATT:
			uk_printf(s, end - s, "%-36s bit 0x%x \t ",
				"CPUS_WAKEUP_FULLBATT ", CPUS_WAKEUP_FULLBATT);
			count++;
			break;
		...
		case CPUS_WAKEUP_POWER_EXP:
			uk_printf(s, end - s, "%-36s bit 0x%x \t ",
				"CPUS_WAKEUP_POWER_EXP", CPUS_WAKEUP_POWER_EXP);
			count++;
			break;
		default:
	...
	for (i = 0; i < 32; i++) {
		...
		if (bit_event) {
			if (bit_event <= WAKEUP_GPIO_PL(GPIO_PL_MAX_NUM)) {
				...
				s += parse_bitmap_gpio(s, end - s, bit_event);
			} else if (bit_event <=
				WAKEUP_GPIO_PM(GPIO_PM_MAX_NUM)) {
				uk_printf(s, end - s, "\tWAKEUP_GPIO_PM ");
				s += parse_bitmap_gpio(s, end - s,
						bit_event >>
						(GPIO_PL_MAX_NUM + 1));
			} else if (bit_event
...
static void __dump_regs(const char *str, u32 *bottom, u32 *top)
{
	u32 *first;
	int i;
...
static void _show_dev_status(char *name, int mask)
{
	u32 *base = 0;
	u32 len = 0;
	...
	if (unlikely(pm_debug_mask & mask)) {
		...
		len /= sizeof(u32);
		__dump_regs(name, base, base + len);

		if (!strncmp(name, "pio", 3)) {
#ifdef CONFIG_PINCTRL_SUNXI_COMMON
...
void show_dev_status(void)
{
	_show_dev_status("pio",    PM_STANDBY_PRINT_IO_STATUS);
	_show_dev_status("r_pio",  PM_STANDBY_PRINT_CPUS_IO_STATUS);
	_show_dev_status("clocks", PM_STANDBY_PRINT_CCU_STATUS);
}
