/*
 * drivers/soc/sunxi/pm/resource/mem_clk.c
...
{
	int size = sizeof(saved_reg) / sizeof(saved_reg[0]);
	void *ccmu_base = (void *)IO_ADDRESS(AW_CCM_BASE);
	int i;

	for (i = 0; i < size; i++)
		saved_reg[i].value = readl(ccmu_base + saved_reg[i].offset);
...
int mem_clk_restore(void)
{
	...
	for (i = 0; i < size; i++) {
		value = readl(ccmu_base + saved_reg[i].offset);
		value &= (~((0x1 << saved_reg[i].src_offset) - 1));
		value |= (saved_reg[i].value & ((0x1 << saved_reg[i].src_offset) - 1));
		writel(value, ccmu_base + saved_reg[i].offset);

		for (j = 0; j < 500000; j++)
			;

		value &= (~(saved_reg[i].src_mask << saved_reg[i].src_offset));
		value |= (saved_reg[i].value & (saved_reg[i].src_mask << saved_reg[i].src_offset));
		writel(value, ccmu_base + saved_reg[i].offset);
...
struct __ccmu_reg_list_t *mem_clk_init(__u32 mmu_flag)
{
	if (mmu_flag == 1)
		...
		CmuReg = (struct __ccmu_reg_list_t *) (AW_CCM_BASE);
...
{
	pclk_state->CmuReg = CmuReg = (struct __ccmu_reg_list_t *)
				IO_ADDRESS(AW_CCM_BASE);
...
__u32 mem_clk_get_cpu_freq(void)
{
	...
	if (reg_val == 0) {
		...
		FactorP = (CmuReg_Pll1Ctl_tmp.bits.FactorP == 0) ? 1 : 4;
...
__s32 mem_clk_save(struct clk_state *ccm_reg)
{
	...
	ccm_reg->ccm_mod_reg = (__ccmu_mod_reg_list_t *)
				IO_ADDRESS(SUNXI_CCM_MOD_PBASE);
...
__s32 mem_clk_restore(struct clk_state *ccm_reg)
{
	...
	/*ccm_reg->ccm_reg->Pll_C0_Bias.dwva = ccm_reg->ccm_reg_backup.Pll_C0_Bias.dwval; */
	ccm_reg->ccm_reg->Pll_Audio_Bias.dwval =
				ccm_reg->ccm_reg_backup.Pll_Audio_Bias.dwval;
	ccm_reg->ccm_reg->Pll_Periph1_Bias.dwval =
				ccm_reg->ccm_reg_backup.Pll_Periph1_Bias.dwval;
	...
	ccm_reg->ccm_reg->Ahb1_Cfg.dwval = (~0x3000000)
				& (ccm_reg->ccm_reg_backup.Ahb1_Cfg.dwval);
	ccm_reg->ccm_reg->Ahb2_Cfg.dwval = (~0x3000000)
				& (ccm_reg->ccm_reg_backup.Ahb2_Cfg.dwval);
	ccm_reg->ccm_reg->Apb0_Cfg.dwval = (~0x3000000)
				& (ccm_reg->ccm_reg_backup.Apb0_Cfg.dwval);
	delay_us(10);
	ccm_reg->ccm_reg->Apb1_Cfg.dwval = (~0x3000000)
				& (ccm_reg->ccm_reg_backup.Apb1_Cfg.dwval);
	ccm_reg->ccm_reg->Cci400_Cfg.dwval = (~0x3000000)
				& (ccm_reg->ccm_reg_backup.Cci400_Cfg.dwval);
	delay_us(10);
