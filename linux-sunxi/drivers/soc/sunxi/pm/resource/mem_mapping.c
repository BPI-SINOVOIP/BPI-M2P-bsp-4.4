/*
 * drivers/soc/sunxi/pm/resource/mem_mapping.c
...
void create_mapping(void)
{
	...
	ttbcr_n = ttbcr & 0x7;
	if (ttbcr_n)
		panic("err: ttbcr.n not == 0, so u can not use ttbr0.\n");
	...
	tlb_item_addr = phys_to_virt((phys_addr_t)
				(ttbr0 + (SRAM_FUNC_START>>18)));
	...
	**/
	...
	dmac_flush_range((void *)(tlb_item_addr),
				(void *)(tlb_item_addr + (sizeof(u32))));
	local_flush_tlb_kernel_range((unsigned long)(tlb_item_addr),
				(unsigned long)(tlb_item_addr + (sizeof(u32))));
	flush_tlb_all();
}
...
void restore_mapping(void)
{
	...
	*((volatile __u32 *)(vaddr)) = backup_tbl[0].entry_val;
	...
	local_flush_tlb_kernel_range((unsigned long)(vaddr),
				(unsigned long)(vaddr + (sizeof(u32))));
}
