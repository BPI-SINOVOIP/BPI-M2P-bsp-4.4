/*
 * drivers/soc/sunxi/pm/standby/standby.c
...
#include "standby_common.h"
#include "standby_serial.h"
#include "standby_twi.h"
#include "dram/mctl_standby.h"
#include "axp/axp_core.h"
...
int standby_main(struct aw_pm_info *arg)
{
	...
	if (pm_info.standby_para.pextended_standby) {
	...
	/* init uart for print */
	if (unlikely(pm_info.standby_para.debug_mask
		& PM_STANDBY_PRINT_STANDBY))
		serial_init_manager();

	/* calc dram checksum */
	if (standby_dram_crc_enable(&(
			extended_standby_para_info.soc_dram_state))) {
		before_crc =
		standby_dram_crc(&(extended_standby_para_info.soc_dram_state));
		mem_preload_tlb();
	}
...
#ifdef CONFIG_AW_AXP
	if (extended_standby_para_info.id == SUPER_STANDBY_FLAG) {
		...
		if (losc_enter_ss != NULL) {
			save_mem_status(before_crc);
			losc_enter_ss();
			asm("b .");
		}

		/* when apb2 clk src is losc, twi clk div ratio need reconfig */
		standby_twi_init_losc(pm_info.pmu_arg.twi_port);
	}

	/* power domain suspend */
	power_domain_suspend(&pm_info, &extended_standby_para_info);
	...
	/* power domain resume */
	power_domain_resume(&extended_standby_para_info);
#endif
...
static int dram_enter_selfresh(extended_standby_t *para)
{
	...
	return dram_power_save_process();
}
...
static int bus_enter_lowfreq(extended_standby_t *para)
{
	...
	if (para->soc_dram_state.selfresh_flag == 1) {
		/* standby_printk("disable HOSC, and disable LDO\n"); */
		/* disable HOSC, and disable LDO */
		if ((para->cpux_clk_state.osc_en & BITMAP(OSC_HOSC_BIT)) == 0)
			standby_clk_hoscdisable();

		if ((para->cpux_clk_state.osc_en & BITMAP(OSC_LDO1_BIT)) == 0)
...
static void cache_count_output(void)
{
	if (d_cache_miss_end || d_tlb_miss_end
		|| i_tlb_miss_end || i_cache_miss_end) {
		standby_printk("====================NOTICE===================.\n");
		standby_printk("d_cache_miss_start = %d, d_cache_miss_end= %d.\n",
				d_cache_miss_start, d_cache_miss_end);
		standby_printk("d_tlb_miss_start = %d, d_tlb_miss_end= %d.\n",
				d_tlb_miss_start, d_tlb_miss_end);
		standby_printk("i_cache_miss_start = %d, i_cache_miss_end= %d.\n",
				i_cache_miss_start, i_cache_miss_end);
		standby_printk("i_tlb_miss_start = %d, i_tlb_miss_end= %d.\n",
				i_tlb_miss_start, i_tlb_miss_end);
		asm("b .");
	} else {
		standby_printk("no miss.\n");
	}
}
