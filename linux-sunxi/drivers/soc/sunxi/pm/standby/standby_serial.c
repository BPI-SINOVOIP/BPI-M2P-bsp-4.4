/*
 * drivers/soc/sunxi/pm/standby/standby_serial.c
...
static __u32 set_serial_clk(__u32 mmu_flag)
{
	...
	struct __ccmu_reg_list_t *ccu_reg = (struct __ccmu_reg_list_t *) (0);
	union __ccmu_apb2_ratio_reg0058_t apb2_reg;

	__u32 port = 0;
	__u32 i = 0;

	if (mmu_flag == 1)
		ccu_reg = (struct __ccmu_reg_list_t *) IO_ADDRESS(AW_CCM_BASE);
	else
		ccu_reg = (struct __ccmu_reg_list_t *) (AW_CCM_BASE);
	...
	/*calculate p2clk. */
	p2clk = src_freq
		/ ((apb2_reg.bits.DivM + 1) * (1 << (apb2_reg.bits.DivN)));
	 ...
	 * the uartdiv is not proper
	 * and the baudrate err exceed the acceptable range.
	 */
	if (mmu_flag) {
		/*backup apb2 gating; */
		backup_ccu_uart = *(volatile unsigned int *)
				(IO_ADDRESS(AW_CCU_UART_PA));
		/*backup uart reset */
		backup_ccu_uart_reset = *(volatile unsigned int *)
...
static __u32 set_serial_clk(__u32 mmu_flag)
{
	...
	ccu_reg = (struct __ccmu_reg_list_t *) mem_get_ba();
	/*check uart clk src is ok or not. */
	/*the uart clk src need to be pll6 & clk freq == 600M? */
	/*so the baudrate == p2clk/(16*div) */
	...
	/*calculate p2clk. */
	p2clk = src_freq / ((ccu_reg->Apb1_Cfg.bits.clk_rat_m + 1)
...
static void set_serial_gpio(__u32 mmu_flag, __u32 port_id)
{
	...
	if (mmu_flag) {
		/*backup gpio */
		backup_gpio_uart = *(volatile unsigned int *)
				(IO_ADDRESS(AW_UART_GPIO_PA));
