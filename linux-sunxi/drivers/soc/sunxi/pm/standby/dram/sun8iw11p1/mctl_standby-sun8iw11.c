 ...
 *
 * Description:  This file implements basic functions for AW1701 DRAM controller
 *
 * History:
...
static unsigned int __dram_power_save_process(void)
{
	...
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 11; j++)
			reg[15 + j + (i * 11)] =
				mctl_read_w((DATX0IOCR(j) + i * 0x80));
	}
 ...
 * return value : Void
 */
void bit_delay_compensation_standby(struct __dram_para_t *para)
{
...
unsigned int auto_cal_timing_standby(unsigned int time_ns, unsigned int clk)
{
	...
	value = (time_ns * clk) / 1000
		+ ((((time_ns * clk) % 1000) != 0) ? 1 : 0);
	return value;
}
 ...
 * return value : void
 */
void auto_set_timing_para_standby(struct __dram_para_t *para)
{
 ...
 * return value : 1
 */
unsigned int ccm_set_pll_ddr1_sscg_standby(struct __dram_para_t *para)
{
	...
	/* swtich to 24M clk source before update PLL */
 ...
 * return value : 1
 */
unsigned int ccm_set_pll_ddr0_sscg_standby(struct __dram_para_t *para)
{
	unsigned int ret, reg_val;
 ...
 */
unsigned int ccm_set_pll_ddr0_clk_standby(struct __dram_para_t *para)
{
	...
	if (div <= 32) {
		...
		/* when m=1, we cann't get absolutely accurate value
		 * for follow clock:
		 * 840(816), 888(864),
		 * 984(960)
		 */
	...
	/* enable PLL_DDR1 HW LOCK */
	rval = mctl_read_w(CCU_PLL_LOCK_CTRL_REG);
	rval |= (0x1 << 4);
	mctl_write_w(rval, CCU_PLL_LOCK_CTRL_REG);
	while (!((mctl_read_w(_CCM_PLL_DDR0_REG)
		& (1 << 20 | 1 << 28)) == (0 << 20 | 1 << 28)))
		;
 ...
 * return value : PLL Frequency
 */
unsigned int ccm_set_pll_ddr1_clk_standby(struct __dram_para_t *para)
{
	unsigned int rval;
	unsigned int div;
	...
	rval |= (0x1 << 11);
	mctl_write_w(rval, CCU_PLL_LOCK_CTRL_REG);
	while (!((mctl_read_w(_CCM_PLL_DDR1_REG)
		& (1 << 30 | 1 << 28)) == (0 << 30 | 1 << 28)))
		;
 ...
 */
unsigned int mctl_sys_init_standby(struct __dram_para_t *para)
{
	unsigned int reg_val = 0;
 ...
 * return value : void
 */
void mctl_com_init_standby(struct __dram_para_t *para)
{
 ...
 */
unsigned int mctl_channel_init_standby(unsigned int ch_index,
				struct __dram_para_t *para)
{
 ...
 * return value : 0-FAIL  , other-Success
 */
unsigned int mctl_core_init_standby(struct __dram_para_t *para)
{
	mctl_sys_init_standby(para);
	mctl_com_init_standby(para);
	auto_set_timing_para_standby(para);

	return mctl_channel_init_standby(0, para);
}
 ...
 */
signed int init_DRAM_standby(int type, struct __dram_para_t *para)
{
	unsigned int ret_val = 0;
	unsigned int reg_val = 0;
 ...
 * return value : 0-FAIL  , other-DRAM size
 */
static unsigned int __dram_power_up_process(struct __dram_para_t *para)
{
	return init_DRAM_standby(0, para);
}
...
unsigned int dram_power_up_process(struct __dram_para_t *para)
{
	return __dram_power_up_process(para);
}
