...
#if defined(CONFIG_WAKEUP_BY_GT5688)
...
#if defined(CONFIG_WAKEUP_BY_GT5688)
static int gt1x_suspend_flag;
#endif

static void resume_worker(struct work_struct *arg)
{
	gt1x_resume();
}
...
int revert_x_flag;
int revert_y_flag;
int ctp_screen_max_x;
int ctp_screen_max_y;
...
static irqreturn_t gt1x_ts_irq_handler(int irq, void *dev_id)
{
	GTP_DEBUG_FUNC();
#if defined(CONFIG_WAKEUP_BY_GT5688)
	__pm_wakeup_event(gt1x_i2c_client->dev.power.wakeup, 500);
	if (gt1x_suspend_flag == 1) {
		return IRQ_HANDLED;
	}
#endif
	gt1x_irq_disable();
...
static int gt1x_parse_dt(struct device *dev)
{
	...
	struct gpio_config int_gpio_config, rst_gpio_config;
	int ret = 0;
	...
	if (of_property_read_u32(np, "ctp_exchange_x_y_flag", &exchange_x2y)) {
		exchange_x2y = 0;
		GTP_ERROR("failed to get ctp_exchange_x_y_flag\n");
	}
	if (of_property_read_u32(np, "ctp_revert_x_flag", &revert_x_flag)) {
		revert_x_flag = 0;
		GTP_ERROR("failed to get revert_x_flag\n");
	}
	if (of_property_read_u32(np, "ctp_revert_y_flag", &revert_y_flag)) {
		revert_y_flag = 0;
		GTP_ERROR("failed to get revert_y_flag\n");
	}
	if (of_property_read_u32(np, "ctp_screen_max_x", &ctp_screen_max_x)) {
		ctp_screen_max_x = 4096;
		GTP_ERROR("failed to get ctp_screen_max_x\n");
	}
	if (of_property_read_u32(np, "ctp_screen_max_y", &ctp_screen_max_y)) {
		ctp_screen_max_y = 4096;
		GTP_ERROR("failed to get ctp_screen_max_y\n");
	}

	vdd_ana = regulator_get(dev, "vcc-ctp");
	if (IS_ERR(vdd_ana)) {
...
static s32 gt1x_request_irq(void)
{
	...
	ret =
	    request_irq(gt1x_i2c_client->irq, gt1x_ts_irq_handler,
			irq_table[gt1x_int_type], gt1x_i2c_client->name,
			gt1x_i2c_client);
	if (ret) {
...
#if GTP_CHANGE_X2Y
	if (exchange_x2y == 1) {
		input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, gt1x_abs_y_max, 0,
			     0);
		input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, gt1x_abs_x_max, 0,
			     0);
	} else {
		input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, gt1x_abs_x_max, 0,
			     0);
		input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, gt1x_abs_y_max, 0,
			     0);
	}
#else
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, gt1x_abs_x_max, 0,
			 ...
			 const struct i2c_device_id *id)
{
...
#endif
	printk(KERN_EMERG "GT5688 PROBE IS FOUND\n");
	...
	INIT_WORK(&resume_work, resume_worker); /*support resume to queue work*/
...
#if GTP_AUTO_UPDATE
	thread =
	    kthread_run(gt1x_auto_update_proc, (void *)NULL,
			"gt1x_auto_update");
	if (IS_ERR(thread)) {
		ret = PTR_ERR(thread);
	...
	enable_wakeup_src(CPUS_GPIO_SRC, gt1x_int_gpio);
...
#if defined(CONFIG_WAKEUP_BY_GT5688)
	disable_wakeup_src(CPUS_GPIO_SRC, gt1x_int_gpio);
...
#if defined(CONFIG_PM) && !defined(CONFIG_WAKEUP_BY_GT5688)
/**
...
#elif defined(CONFIG_PM) && defined(CONFIG_WAKEUP_BY_GT5688)
/*Beind called in resume_worker to clear gt1x_irq_pending*/
static int gt1x_pm_suspend(struct device *dev)
{
	gt1x_suspend_flag = 1;
	return 0;
