			       ...
			       size_t len, loff_t *data)
{
	...
	if (1 == cmd_head.wr) {
		...
		gt1x_enter_update_mode(); else if (13 == cmd_head.wr)
		gt1x_leave_update_mode(); else if (15 == cmd_head.wr) {
		...
		thrd =
		    kthread_run(gt1x_update_firmware, (void *)cmd_head.data,
				"GT1x FW Update");
		if (IS_ERR(thrd))
			return PTR_ERR(thrd);
	}
			      ...
			      size_t count, loff_t *ppos)
{
	...
	if (cmd_head.wr % 2) {
		...
		while (data_len > 0) {
			...
			memcpy(&buffer[loc], &cmd_head.data[GTP_ADDR_LENGTH],
			       len);
			data_len -= len;
			addr += len;
			loc += len;
		...
		*ppos += cmd_head.data_len;
		...
		GTP_DEBUG("Return ic type:%s len:%d.", buffer,
