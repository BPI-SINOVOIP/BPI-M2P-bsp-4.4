...
int gt1x_auto_update_proc(void *data)
{
	...
	if (ret & (FOUND_FW_PATH_1 | FOUND_FW_PATH_2)) {
		if (ret & FOUND_FW_PATH_1)
			filename = UPDATE_FILE_PATH_1; else
	...
	if (ret & (FOUND_CFG_PATH_1 | FOUND_CFG_PATH_2)) {
		if (ret & FOUND_CFG_PATH_1)
			filename = CONFIG_FILE_PATH_1; else
			filename = CONFIG_FILE_PATH_2;

		if (gt1x_parse_config(filename, config) > 0) {
			if (gt1x_i2c_write
			    (GTP_REG_CONFIG_DATA, config,
			     GTP_CONFIG_MAX_LENGTH))
				GTP_ERROR("Update config failed!"); else
...
int gt1x_update_firmware(void *filename)
{
	...
	update_info.max_progress = 6 + update_info.firmware->subsystem_count;
#else
	update_info.max_progress = 3 + update_info.firmware->subsystem_count;
#endif
	...
	for (i = 1; i < update_info.firmware->subsystem_count; i++) {
		...
		ret =
		    gt1x_burn_subsystem(&(update_info.firmware->subsystem[i]));
		if (ret) {
	...
	for (i = 1; i < update_info.firmware->subsystem_count; i++) {
		...
		ret =
		    gt1x_check_subsystem_in_flash(&
						  (update_info.
						   firmware->subsystem[i]));
		if (ret) {
			gt1x_error_erase();
			break;
		}
...
int gt1x_update_prepare(char *filename)
{
	...
	if (filename == NULL) {
		...
		update_info.fw_file =
		    filp_open(update_info.fw_name, O_RDONLY, 0);
		if (IS_ERR(update_info.fw_file)) {
			GTP_ERROR("Open update file(%s) error!",
				  update_info.fw_name);
			set_fs(update_info.old_fs);
			return ERROR_FILE;
		}
		update_info.fw_file->f_op->llseek(update_info.fw_file, 0,
						  SEEK_SET);
	...
	while (retry > 0) {
		...
		update_info.firmware =
		    (struct fw_info *)kzalloc(sizeof(struct fw_info),
					      GFP_KERNEL);
		if (update_info.firmware == NULL) {
			...
			GTP_INFO("Alloc %zu bytes memory success.",
				 sizeof(struct fw_info));
			break;
		}
	}
...
int gt1x_check_firmware(void)
{
	...
	GTP_INFO("Update type: %s",
		 ...
		 UPDATE_TYPE_HEADER ? "Header" : "File");
...
u8 *gt1x_get_fw_data(u32 offset, int length)
{
	int ret;
	if (update_info.update_type == UPDATE_TYPE_FILE) {
		update_info.fw_file->f_op->llseek(update_info.fw_file, offset,
						  SEEK_SET);
		ret =
		    update_info.fw_file->f_op->read(update_info.fw_file,
						    (char *)update_info.buffer,
						    length,
						    &update_info.
						    fw_file->f_pos);
		if (ret < 0) {
...
_reset:
	...
	if (reg_val[0] != 0xBE || reg_val[1] == 0xAA) {
		GTP_INFO
		    ("Check fw status reg not pass,reg[0x814E]=0x%2X,reg[0x5095]=0x%2X!",
		     reg_val[0], reg_val[1]);
		return 0;
	}
...
int gt1x_hold_ss51_dsp(void)
{
	...
	buffer[0] = 0x00;
	ret =
	    gt1x_i2c_write_with_readback(_bRW_MISCTL__DSP_MCU_PWR_, buffer, 1);
	if (ret) {
	...
	buffer[0] = 0x00;
	ret =
	    gt1x_i2c_write_with_readback(_rRW_MISCTL__BOOT_OPT_B0_, buffer, 1);
	if (ret) {
...
int gt1x_run_ss51_isp(u8 *ss51_isp, int length)
{
	...
	buffer[0] = 0x01;
	ret =
	    gt1x_i2c_write_with_readback(_bRW_MISCTL__PATCH_AREA_EN_, buffer,
					 1);
	if (ret) {
...
int gt1x_burn_subsystem(struct fw_subsystem_info *subsystem)
{
	...
	while (length > 0 && retry > 0) {
		...
		ret =
		    gt1x_i2c_write_with_readback(0x8100 + 4 + block_len, buffer,
						 2);
		if (ret) {
		...
		while (wait_time-- > 0) {
			...
			if (buffer[0] == 0xAA) {
				...
				GTP_DEBUG("unknown state!(0x8022: 0x%02X)",
					  buffer[0]);
			}
		}
...
int gt1x_check_subsystem_in_flash(struct fw_subsystem_info *subsystem)
{
	...
	while (length > 0) {
		...
		if (fw == NULL)
			return ERROR_FW;
		ret =
		    gt1x_read_flash(subsystem->address + checked_len,
				    block_len);
		if (ret)
			check_state |= ret;
	...
	if (check_state)
		GTP_ERROR("Subsystem in flash is broken!"); else
...
int gt1x_load_patch(u8 *patch, u32 patch_size, int offset, int bank_size)
{
	...
	GTP_INFO
	    ("Load patch code(size: %d, checksum: 0x%04X, position: 0x%04X, bank-size: %d",
	     patch_size, gt1x_calc_checksum(patch, patch_size), 0xC000 + offset,
	     bank_size);
	while (loaded_length != patch_size) {
		...
		len =
		    ...
		    GT1X_LOAD_PACKET_SIZE ? GT1X_LOAD_PACKET_SIZE : patch_size -
		    loaded_length;
		...
		if (ret) {
			GTP_ERROR("Recall check 0x%04X, %dbytes fail!", address,
				  len);
			return ret;
		}
