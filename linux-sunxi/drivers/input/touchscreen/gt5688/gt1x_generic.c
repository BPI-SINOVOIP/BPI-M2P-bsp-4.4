...
extern int exchange_x2y;
extern int revert_x_flag;
extern int revert_y_flag;

extern int ctp_screen_max_x;
extern int ctp_screen_max_y;
				    ...
				    size_t size, loff_t *ppos)
{
	...
	ptr +=
	    sprintf(ptr, "ProductID: GT%c%c%c%c\n", temp_data[0], temp_data[1],
...
static ssize_t gt1x_debug_write_proc(struct file *file, const char *buffer,
				     size_t count, loff_t *ppos)
{
	...
	if (count == gt1x_cfg_length) {
		...
		gt1x_abs_x_max =
		    (gt1x_config[RESOLUTION_LOC + 1] << 8) +
		    gt1x_config[RESOLUTION_LOC];
		gt1x_abs_y_max =
		    (gt1x_config[RESOLUTION_LOC + 3] << 8) +
...
s32 _do_i2c_read(struct i2c_msg *msgs, u16 addr, u8 *buffer, s32 len)
{
	...
	data =
	    (u8 *) kmalloc(IIC_MAX_TRANSFER_SIZE <
			   (len +
			    GTP_ADDR_LENGTH) ? IIC_MAX_TRANSFER_SIZE : (len +
									GTP_ADDR_LENGTH),
			   GFP_KERNEL);
	if (data == NULL)
		return ERROR_MEM;
	...
	while (pos != data_length) {
		if ((data_length - pos) > IIC_MAX_TRANSFER_SIZE)
			transfer_length = IIC_MAX_TRANSFER_SIZE; else
...
static s32 gt1x_i2c_test(void)
{
	...
	while (retry++ < 3) {
		ret =
		    gt1x_i2c_read(GTP_REG_HW_INFO, (u8 *) &hw_info,
				  sizeof(hw_info));
		if (!ret) {
...
s32 gt1x_init_panel(void)
{
	...
	if (cfg_len < GTP_CONFIG_MIN_LENGTH || cfg_len > GTP_CONFIG_MAX_LENGTH) {
		GTP_ERROR
		    ("Config group%d is INVALID! You need to check you header file CFG_GROUP section!",
		     sensor_id + 1);
		return -1;
	}
...
s32 gt1x_read_version(struct gt1x_version_info *ver_info)
{
	...
	while (retry--) {
		ret =
		    gt1x_i2c_read_dbl_check(GTP_REG_VERSION, buf, sizeof(buf));
		if (!ret) {
			checksum = 0;
...
s32 gt1x_get_chip_type(void)
{
	...
	/* read hardware */
	ret =
	    gt1x_i2c_read_dbl_check(GTP_REG_HW_INFO, opr_buf, sizeof(opr_buf));
	if (ret) {
	...
	if (!memcmp(opr_buf, gt1x_data, sizeof(gt1x_data)))
		gt1x_chip_type = CHIP_TYPE_GT1X; else if (!memcmp(opr_buf, gt9l_data, sizeof(gt9l_data)))
...
s32 gt1x_request_event_handler(void)
{
	...
	case GTP_RQST_CONFIG:
		...
		ret = gt1x_send_cfg(gt1x_config, gt1x_cfg_length);
		if (ret)
			GTP_ERROR("Send gt1x_config error."); else {
			     ...
			     struct input_dev *pen_dev)
{
	...
	if (touch_num > 1) {
		ret =
		    gt1x_i2c_read((GTP_READ_COOR_ADDR + 11), &touch_data[11],
				  1 + 8 * touch_num + 2 - 11);
		if (ret)
			return ret;
	}
	...
	if (check_sum) {	/* checksum error */
		ret =
		    gt1x_i2c_read(GTP_READ_COOR_ADDR, touch_data,
				  3 + 8 * touch_num);
		if (ret)
			return ret;
	...
	if ((touch_data[0] & 0x10) && key_value) {
		...
		if (key_value & 0xF0)
			SET_BIT(cur_event, BIT_STYLUS_KEY); else if (key_value & 0x0F)
	...
	if (unlikely(!pre_event && !cur_event))
		GTP_DEBUG("Additional Int Pulse."); else
		pre_event = cur_event;
...
static s32 gt1x_ps_enable(s32 enable)
{
	...
	if (!ret && enable)
		gt1x_ps_dev->enabled = 1; else
		gt1x_ps_dev->enabled = 0;
	gt1x_ps_dev->state = PS_FARAWAY;
	GTP_INFO("Proximity function %s %s.", state ? "enable" : "disable",
		 ret ? "fail" : "success");
	return ret;
}
				   ...
				   size_t count)
{
	...
	if (input == 1)
		gt1x_ps_dev->state = PS_FARAWAY; else if (input == 0)
		gt1x_ps_dev->state = PS_NEAR; else
		return -EINVAL;
...
static struct kobj_attribute ps_attrs[] = {
	__ATTR(enable, S_IWUGO | S_IRUGO, gt1x_ps_enable_show,
	       gt1x_ps_enable_store),
	__ATTR(state, S_IWUGO | S_IRUGO, gt1x_ps_state_show,
	       gt1x_ps_state_store)
};
...
static int gt1x_ps_init(void)
{
	...
	if (!sysfs_rootdir) {
		...
		if (!sysfs_rootdir) {
			GTP_ERROR
			    ("Failed to create and add sysfs interface: goodix.");
			err = -ENOMEM;
			goto err_register_dev;
		}
	}
	...
	if (!gt1x_ps_dev->kobj) {
		GTP_ERROR
		    ("Failed to create and add sysfs interface: proximity.");
		err = -ENOMEM;
		goto err_register_dev;
	}
	/*   create sysfs files */
	{
		...
		for (i = 0; i < sizeof(ps_attrs) / sizeof(ps_attrs[0]); i++) {
			if ((err ==
			     sysfs_create_file(gt1x_ps_dev->kobj,
					       &ps_attrs[i].attr)))
...
static int gt1x_smart_cover_init(void)
{
	...
	if (!gt1x_sc_dev->kobj)
		gt1x_sc_dev->kobj =
		    kobject_create_and_add("smartcover", sysfs_rootdir);
	if (!gt1x_sc_dev->kobj) {
		err = -3;
		goto exit_free_mem;
	}
...
#if GTP_SMART_COVER
	if (gt1x_sc_dev && gt1x_sc_dev->enabled && gt1x_sc_dev->state)
		return;
#endif
...
#if defined(GTP_POWER_CTRL_SLEEP)
#define PB_CFG_1 0X300B028
/*Being called in gt1x_resume in order to support GTP_INT_PORT to eint */
static void gt1x_gpio_to_irq(void)
{
	int *pb_external_int = ioremap(PB_CFG_1, sizeof(int));

	/* printk("pb_external_int: 0x%x\n", *pb_external_int);
	 * pb_external_int = 1 << 9; //clear PB9 IRQ pending
	   iounmap(pb_external_int);*/
	*pb_external_int &= 0xffffff0f;
	*pb_external_int |= 0x6 << 4;  /*config PB9 to PB_EINT9*/
...
#else
		ret =
		    gt1x_i2c_read_dbl_check(GTP_REG_HN_PAIRED, buf,
					    sizeof(buf));
...
s32 gt1x_init(void)
{
	...
	while (retry++ < 5) {
		...
		ret =
		    gt1x_i2c_read_dbl_check(GTP_REG_FW_CHK_MAINSYS, reg_val, 1);
		if (ret != 0)
			continue; else if (reg_val[0] != 0xBE) {
		...
		/* debug info  */
		ret =
		    gt1x_i2c_read_dbl_check(GTP_REG_FW_CHK_SUBSYS, reg_val, 1);
