...
static struct workqueue_struct *msg2xxx_wq;
/*=============================================================*/
// VARIABLE DECLARATION
/*=============================================================*/

extern unsigned int sunxi_suspend_flag;
extern struct i2c_client *g_I2cClient;

extern int revert_x_flag;
extern int revert_y_flag;
extern int exchange_x2y;
extern int tpd_abs_x_max;
extern int tpd_abs_y_max;

#if defined(CONFIG_WAKEUP_BY_MSG2XXX)
extern int gt1x_suspend_flag;
#endif
...
void DrvFingerTouchPressed(s32 nX, s32 nY, s32 nPressure, s32 nId)
{
	...
	if (revert_x_flag == 1)
		nX = GTP_WARP_X(tpd_abs_x_max, nX);
	if (revert_y_flag == 1)
		nY = GTP_WARP_Y(tpd_abs_y_max, nY);
...
static irqreturn_t _DrvFingerTouchInterruptHandler(s32 nIrq, void *pDeviceId)
{
	...
	spin_lock_irqsave(&_gIrqLock, nIrqFlag);
#if defined(CONFIG_WAKEUP_BY_MSG2XXX)
	__pm_wakeup_event(g_I2cClient->dev.power.wakeup, 500);
	if (sunxi_suspend_flag == 1) {
		spin_unlock_irqrestore(&_gIrqLock, nIrqFlag);
		return IRQ_HANDLED;
	}
#endif
    ...
    ) 
    {
		...
		queue_work(msg2xxx_wq, &_gFingerTouchWork);
	}
	spin_unlock_irqrestore(&_gIrqLock, nIrqFlag);
...
s32 DrvTouchDeviceRegisterFingerTouchInterruptHandler(void)
{
	...
	msg2xxx_wq = create_singlethread_workqueue("msg2xxx_wq");
...
void MsDrvInterfaceTouchDeviceResume(void)
{
    ...
    irq_r = request_irq(_gIrq/*MS_TS_MSG_IC_GPIO_INT*/, _DrvFingerTouchInterruptHandler,
                      /*IRQF_TRIGGER_RISING*/IRQF_TRIGGER_FALLING/* | IRQF_NO_SUSPEND */,
                      "msg2xxx", NULL); 
	disable_irq_nosync(_gIrq);
