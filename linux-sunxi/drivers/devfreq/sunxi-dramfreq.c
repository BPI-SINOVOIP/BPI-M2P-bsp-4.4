...
#include "governor_temptrigger.h"
#endif
...
#define DRAMFREQ_DBG(mask, format, args...)			\
	do {							\
		if (mask & debug_mask)				\
			pr_err("[dramfreq] "format, ##args);	\
	} while (0)

#define DRAMFREQ_ERR(format, args...) \
	pr_err("[dramfreq] ERR:"format, ##args)
				...
				unsigned long *freq, u32 flags)
{
	...
	jump = (valid_freq > dramfreq->devfreq->previous_freq) ?
		FREQ_UP : FREQ_DOWN;
	...
	if ((dramfreq->devfreq->previous_freq == dramfreq->devfreq->min_freq) &&
		...
		for (i = ARRAY_SIZE(busfreq_tbl) - 1; i >= 0; i--) {
			sunxi_dramfreq_busfreq_target(busfreq_tbl[i].name,
						busfreq_tbl[i].bus_clk,
						busfreq_tbl[i].normal_freq);
		}
	...
	if (dramfreq->mode == DFS_MODE) {
		...
		if (!(dramfreq_vbtime_ops.get_next_vb_time
			...
			DRAMFREQ_ERR("dramfreq_vbtime_ops is illegal!\n");
			ret = -EINVAL;
			goto unlock;
		}
		...
		if (next_vb_retries >= MDFS_LEAST_TIME_RETRIES) {
			DRAMFREQ_ERR("Retrying next vb time failed!\n");
			ret = -EINVAL;
			goto unlock;
		} else {
	...
	do {
		ret = sunxi_dramfreq_set_rate(jump, valid_freq, dramfreq,
						masters_access);
		if (ret == -EAGAIN)
			retries++;
		else
...
static int sunxi_dramfreq_governor_state_update(char *name,
						enum GOVERNOR_STATE type)
{
