...
void enable_gpu_clk(void)
{
	...
	if (aw_private.power_gate_addr) {
		writel(0x0, aw_private.power_gate_addr);
	}
...
static void enable_gpu_power(void)
{
	...
	if (aw_private.power_gate_addr) {
		writel(0x1, aw_private.power_gate_addr);
	}
...
static void aw_init(struct platform_device *device)
{
	...
	aw_private.power_gate_addr = ioremap(GPU_POWER_GATING_REG, 1);
...
#ifdef GPU_POWER_GATING_REG
	iounmap(aw_private.power_gate_addr);
#endif /*GPU_POWER_GATING_REG*/
